...
- **모달 접근성 확산 (D-026)**: 폼/다이얼로그형 전용 모달 8종(LAKE 서비스 등록·서비스 등록·
  프로젝트 폼·클러스터 정보 수집 diff·이미지 배포·플레이북 로그·배치잡 등록 위저드·배치잡
  슬라이드오버 overlay)에 공용 `useModalA11y`(Escape 닫기·포커스 트랩·초점 이동/복원)를
  적용하고 `role="dialog"`·`aria-modal`·`aria-labelledby`(또는 aria-label)를 부여했다. 기존의
  수동 Escape 리스너는 훅으로 통합해 중복을 제거했다.
