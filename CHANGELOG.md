...
- **업무 등록/수정 폼에 "전체 참석" 체크박스가 없던 문제**: `WorkItem.allAttendees` 필드는
  백엔드·타입·상세보기 배지(`👥 전체 참석`)·홈 "전체" 카드 필터까지 전부 연결돼 있었지만,
  정작 등록/수정 폼에 이 값을 켤 수 있는 UI 컨트롤이 없어 사용자가 지정할 방법이 없었다
  (state 는 있지만 어떤 `onChange` 도 호출하지 않는 죽은 필드). `WorkItemForm.tsx` 에
  체크박스를 추가해 실제로 지정 가능하도록 수정.
