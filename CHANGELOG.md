...
### Changed
- **버전 정정 (1.36.2 → 1.37.0)**: 1.36.2 에는 auto-release 가 scope 붙은 제목(`feat(ui): …`)을 스킵하던
...
- **움버 테마 (바탕 2종)**: 입체미래주의풍 유화에서 추출한 팔레트로 만든 `umber`(움버 바탕 + 오커 강조 + 더스티
  핑크 차트)와 `umber-light`(크림 바탕 + 움버 사이드바). 오커와 겹치던 경고색은 주황으로 옮기고 핑크는 차트·장식
  전용으로 둬 상태색과 섞이지 않는다. 본문·보조 글자·버튼·링크·상태 글자 모두 4.5:1 이상, 입력 테두리 3:1 이상.
  Frontend: `index.css`(`html.umber`/`html.umber-light`), `themeStore.ts`, 사용자 메뉴·테마 갤러리.
- **플래스터 테마 (바탕 1종)**: 흰 벽 입체주의 풍경화에서 추출한 팔레트로 만든 `plaster` — 크림 석고 바탕 +
  슬레이트 블루 강조 + 파스텔(핑크·라일락·밀짚 노랑) 차트 + 나무 액자 브라운 사이드바. 상태색도 그림에서 골랐고
  (세이지·황토·크림슨·파우더 블루) 입력 테두리는 그림의 시에나 윤곽선. 글자 대비 4.9:1 이상, 입력 테두리 3.1:1.
  Frontend: `index.css`(`html.plaster`), `themeStore.ts`, 사용자 메뉴·테마 갤러리.
- **〈아침 식사〉·릴리프 테마 (바탕 2종)**: 입체주의 정물화 〈아침 식사〉에서 추출한 `journal`(검정 바탕 + 코발트 강조 + 비취 초록 +
  오커 + 크림슨 사이드바, 다크)과 석재 부조에서 추출한 `relief`(스톤 베이지 바탕 + 슬레이트 강조·사이드바 + 녹청
  초록, 라이트). 두 그림 모두 초록이 강해 primary 는 블루 계열로 두고 초록은 정상 상태·차트에만 써 상태색과 섞이지
  않는다. 글자 대비 4.9:1 이상, 입력 테두리 3.3:1. Frontend: `index.css`(`html.journal`/`html.relief`), `themeStore.ts`.
- **〈할리퀸과 목걸이를 한 여인〉 테마 (바탕 1종)**: 입체주의 인물화 〈할리퀸과 목걸이를 한 여인〉에서 추출한 `harlequin` — 은회색 바탕 + 콘플라워 블루 강조(버튼 글자
  검정) + 검정 사이드바. 무채색 위 블루 하나만 색이라 단일 강조 원칙과 맞는다. 글자 대비 5.0:1 이상, 입력 테두리 4.7:1.
- **〈오장팡 자개〉 테마 (바탕 1종)**: 오장팡의 퓨리즘 정물화에서 추출한 `ozenfant` — 자개빛 펄 그레이 바탕 + 스틸 블루 강조(버튼
  글자 흰색) + 진주 블루 사이드바, 위험 상태는 그림 속 산화철 레드. 글자 대비 5.2:1 이상, 입력 테두리 3.8:1.
  Frontend: `index.css`(`html.ozenfant`), `themeStore.ts`, 사용자 메뉴·테마 갤러리.
- **〈김환기 창〉 테마 (바탕 1종)**: 김환기의 추상화 〈창〉에서 추출한 `whanki` — 청자빛 회녹색 바탕 + 프러시안 블루 강조(버튼 글자
  흰색) + 먹빛 녹흑 사이드바. 그림의 진홍·황토·올리브를 위험·경고·정상 상태색에 그대로 대응시켰다. 글자 대비 5.2:1 이상,
...
- **컴포트 테마에서 날짜 입력칸 달력 아이콘이 반전돼 보이던 문제**: 다크 전용 반전 규칙이
  `html:not(.light)` 라 라이트 계열인 컴포트에도 걸렸다. `html.dark` 로 좁혔다.
...
- **테마 11종 → 바탕 4종 + 강조색 6종**: 바탕은 라이트(기본)·다크·컴포트·고대비(신규) + 시스템만 남기고,
  색 취향은 강조색(블루·틸·그린·앰버·코랄·바이올렛)으로 고른다 — 버튼·링크·선택 탭·활성 메뉴·포커스 링만
  바뀌고 배경·상태색은 바탕이 정한다. 6종 모두 라이트·다크에서 버튼 글자·링크 4.5:1 이상. 없어진 테마
  (코랄·번트 시에나·토스카나 선셋·일렉트로팝·서머 브리즈·와일드플라워 메도우·트로피컬 펀치)를 쓰던
  브라우저는 첫 로드 때 가장 가까운 바탕+강조색으로 한 번 옮긴다. 고대비는 관제실 대형 화면용(본문 21:1,
  경계 4.8:1, 포커스 링 3px). Frontend: `themeStore.ts`(`Accent`, `LEGACY_THEME_MAP`), `index.css`
  (`html.high-contrast`, `html.light|dark[data-accent]`, 옛 테마 블록 삭제), 사용자 메뉴·테마 갤러리의 강조색 선택.
...
### Fixed
- **배치잡 "즉시 실행"이 꺼둔(enabled=false) 잡도 실행되던 버그**: 스케줄 디스패처와 일괄
  실행은 `enabled` 를 지켰지만, 단일 잡 "즉시 실행"(테이블 ▶ 아이콘/슬라이드오버 "실행" 폼)이
  호출하는 `POST /batch-jobs/{id}/run` 엔드포인트만 이 검사가 빠져 있어 꺼둔 잡도 클릭 한
  번으로 실행할 수 있었다. Backend: `routers/batch_jobs.py` 의 `run_job()` 에 `enabled` 가드
  추가(400). Frontend: `BatchJobRow`/`BatchJobSlideOver.RunForm` 도 서버와 동일하게 버튼을
  비활성화하고 사유를 안내.
- **점검 매트릭스 셀 상세 "실행 방식" 탭 — 실행 중 단계 색이 표시되지 않던 버그**: 실행이
  진행 중일 때 `ExecutionStepsTimeline` 에 넘기는 `steps` 를 의도적으로 `undefined` 로 지워
  "재생"을 눌러도 전부 회색(대기)으로만 보였다. 백엔드가 실행 중인 단계도 이미 `running`
  상태로 채워 보내고(2초 폴링) 있었으므로 그 값을 그대로 넘기도록 수정 — 이제 완료를 기다리지
  않고도 단계별 진행이 실시간으로 색칠된다.

### Changed
- **배치잡 활성화/비활성화 UI 명확화**: 꺼진 잡은 목록에 `PowerOff` 아이콘 + "사용 안 함"
  배지로 눈에 띄게 표시되고, 즉시 실행 버튼·실행 폼 모두 "왜 실행할 수 없는지"(꺼짐 vs
  저장된 자격증명 없음)를 구분해 안내한다. "꺼짐"의 의미(앞으로의 새 실행만 차단, 기존
  K8s 리소스는 건드리지 않음)를 문구로 명시.
- **점검 매트릭스 — 영역(카테고리)별 그룹핑 + 접기**: 점검 항목이 늘어날수록 admin 이
  한눈에 훑기 어려워지던 문제를 해소 — 행을 category(K8s/네트워크/스토리지/OS/애플리케이션)
  기준으로 묶어 그룹 헤더에 정상/경고/위험 카운트 배지를 상시 노출하고, 기본은 접되 위험/경고가
  있는 그룹만 자동으로 펼친다. 기존 "화면당 표시 행 수" 페이지네이션은 이 그룹 접기로
  대체됐다(표시 설정에 "모두 펼치기"/"모두 접기" 추가).
- **점검 매트릭스 셀 상세 "실행 방식" 탭 — 스크롤 최소화 레이아웃**: 대상/최근 수행 상태를
