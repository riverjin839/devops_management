...
| `html.harlequin` | 〈할리퀸과 목걸이를 한 여인〉(서명 "Picasso, Rome 1917")에서 추출 — 은회색 바탕(`30 2% 88%`) · 콘플라워 블루 primary(`211 45% 62%`, 검정 글자) · 검정 사이드바 | 라이트. 자체 색 고정 |
| `html.ozenfant` | 오장팡 퓨리즘 정물화(서명 "ozenfant") 〈오장팡 자개〉에서 추출 — 자개빛 펄 그레이 바탕(`210 12% 92%`) · 스틸 블루 primary(`210 38% 42%`, 흰 글자) · 진주 블루 사이드바 · 산화철 레드 critical | 라이트. 자체 색 고정 |
| `html.whanki` | 김환기 〈창〉(서명 "whanki")에서 추출 — 청자빛 회녹색 바탕(`170 10% 90%`) · 프러시안 블루 primary(`203 55% 36%`, 흰 글자) · 먹빛 녹흑 사이드바 · 진홍 critical / 황토 warning / 올리브 healthy | 라이트. 자체 색 고정 |
| `html.picasso-girl` | 피카소 〈소녀의 머리〉에서 추출 — 석고 회백 바탕(`38 8% 89%`) · 움버 primary(`30 38% 32%`, 흰 글자) · 먹 검정 사이드바 · 경고는 머스터드(hue 45)로 움버와 분리 | 라이트. 자체 색 고정 |
| `html.picasso-portrait` | 피카소 〈소녀의 초상〉(1914)에서 추출 — 에메랄드 바탕(`155 26% 11%`) · 페리윙클 primary(`228 50% 72%`, 짙은 초록 글자) · 마룬 꽃무늬 사이드바 · 점묘 레드 critical | 다크. 자체 색 고정 |
| `html.picasso-violin` | 피카소 〈바이올린〉(BASS 라벨)에서 추출 — 분필 흰 바탕(`40 10% 91%`) · 바이올렛 primary(`272 32% 44%`, 흰 글자) · 초콜릿 사이드바 + 연노랑 활성 표시 · 주홍 critical | 라이트. 자체 색 고정 |
| `html.gris-guitar` | 후안 그리스 〈기타〉에서 추출 — 민트 바탕(`150 16% 91%`) · 데님 블루 primary(`208 42% 38%`, 흰 글자) · 네이비 사이드바 · 마룬 critical / 살몬 warning | 라이트. 자체 색 고정 |
