...
const THEME_LABEL: Record<Theme, string> = {
  ...
  journal: '저널', relief: '릴리프', system: '시스템',
};
  ...
  return (
    <MacCard title="테마 갤러리" bodyPadding="p-4">
      ...
      <p className="text-xs text-muted-foreground mb-2">
        버튼·링크·선택 탭·활성 메뉴에만 쓰인다. 라이트·다크(시스템 포함) 바탕에 적용되고, 컴포트·고대비와 그림 테마(움버·플래스터·저널·릴리프)는 자체 색을 쓴다.
      </p>
        ...
        {ACCENTS.map((a) => {
          ...
          return (
            <button
              ...
              disabled={!accentOn}
              onClick={() => setAccent(a)}
              title={accentOn ? `강조색 ${ACCENT_LABEL[a]}` : '그림 테마(움버·플래스터·저널·릴리프)와 컴포트·고대비 바탕은 자체 색을 써서 강조색을 바꿀 수 없다'}
