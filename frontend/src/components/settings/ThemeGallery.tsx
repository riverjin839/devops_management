...
const THEME_LABEL: Record<Theme, string> = {
  ...
  journal: '아침 식사', relief: '릴리프', harlequin: '할리퀸과 목걸이를 한 여인', ozenfant: '오장팡 자개', system: '시스템',
};
  ...
  return (
    <MacCard title="테마 갤러리" bodyPadding="p-4">
      ...
      <p className="text-xs text-muted-foreground mb-2">
        버튼·링크·선택 탭·활성 메뉴에만 쓰인다. 라이트·다크(시스템 포함) 바탕에 적용되고, 컴포트·고대비와 그림 테마(움버·플래스터·아침 식사·릴리프·할리퀸과 목걸이를 한 여인·오장팡 자개)는 자체 색을 쓴다.
      </p>
        ...
        {ACCENTS.map((a) => {
          ...
          return (
            <button
              ...
              disabled={!accentOn}
              onClick={() => setAccent(a)}
              title={accentOn ? `강조색 ${ACCENT_LABEL[a]}` : '그림 테마(움버·플래스터·아침 식사·릴리프·할리퀸과 목걸이를 한 여인·오장팡 자개)와 컴포트·고대비 바탕은 자체 색을 써서 강조색을 바꿀 수 없다'}
