...
const THEME_LABEL: Record<Theme, string> = {
  light: '라이트 (기본)', dark: '다크', comfort: '컴포트', 'high-contrast': '고대비', system: '시스템',
};
  ...
  return (
    <MacCard title="테마 갤러리" bodyPadding="p-4">
      ...
      <p className="text-xs text-muted-foreground mb-2">
        버튼·링크·선택 탭·활성 메뉴에만 쓰인다. 라이트·다크(시스템 포함) 바탕에 적용되고, 컴포트·고대비는 자체 색을 쓴다.
      </p>
        ...
        {ACCENTS.map((a) => {
          ...
          return (
            <button
              ...
              disabled={!accentOn}
              onClick={() => setAccent(a)}
              title={accentOn ? `강조색 ${ACCENT_LABEL[a]}` : '컴포트·고대비 바탕에서는 강조색을 바꿀 수 없다'}
