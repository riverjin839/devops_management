      ...
      <div className="relative bg-card border border-border rounded-2xl mac-shadow w-full max-w-lg mx-4 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 px-5 pt-5 pb-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-muted-foreground">내 PAT 권한으로 이슈를 work item 으로 가져옵니다.</p>
          </div>
        ...
        <div className="px-5 pb-5 space-y-3.5">
          ...
          <div className="flex items-stretch gap-1.5">
            {SCOPES.map((s) => (
              <button key={s.id} type="button" onClick={() => setScope(s.id)} aria-pressed={scope === s.id}
                className={`flex-1 px-2 py-2 rounded-xl border text-[11px] font-medium transition-colors ${
          ...
          {scope === 'jql' && (
            <div>
              <span className="text-xs font-medium text-muted-foreground mb-1 block">JQL</span>
          ...
          {preview && preview.status === 'ok' && (
            <div className="rounded-xl border border-border bg-secondary/30 p-3 text-xs">
              ...
              {preview.dryRun && <p className="text-[10px] text-muted-foreground mt-1">미리보기 — 아직 저장되지 않았습니다.</p>}
                ...
                <ul className="mt-2 max-h-40 overflow-y-auto divide-y divide-border/40">
                  {preview.items.map((it) => (
                    <li key={it.jiraKey} className="py-1 flex items-center gap-2">
                      <span className={`text-[9px] font-semibold px-1 rounded ${it.action === 'create' ? 'bg-emerald-500/15 text-emerald-500' : 'bg-blue-500/15 text-blue-500'}`}>
                        {it.action === 'create' ? '신규' : '갱신'}
                      </span>
                      <span className="font-mono text-[10px] text-muted-foreground">{it.jiraKey}</span>
          ...
          {preview && preview.status !== 'ok' && (
            <div className="rounded-xl bg-red-500/10 text-red-500 px-3 py-2 text-xs flex items-center gap-1.5">
