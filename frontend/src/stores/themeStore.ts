 ...
 */
export type Theme = 'light' | 'dark' | 'comfort' | 'high-contrast' | 'umber' | 'umber-light' | 'plaster' | 'journal' | 'relief' | 'harlequin' | 'ozenfant' | 'system';
export type Accent = 'blue' | 'teal' | 'green' | 'amber' | 'coral' | 'violet';

export const THEMES: readonly Theme[] = ['light', 'dark', 'comfort', 'high-contrast', 'umber', 'umber-light', 'plaster', 'journal', 'relief', 'harlequin', 'ozenfant', 'system'];
...
const STANDALONE_THEMES = ['comfort', 'high-contrast', 'umber', 'umber-light', 'plaster', 'journal', 'relief', 'harlequin', 'ozenfant'] as const;
...
/** 강조색이 적용되는 바탕인지 — comfort/high-contrast/umber(-light)/plaster/journal/relief/harlequin/ozenfant 는 자체 색 고정이라 강조색을 무시한다. */
