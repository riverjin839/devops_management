...
| Jira 연동 업무 → 사내 ServiceNow ITSM 수동 등록 (1차 구현 — Jira/SSO 세션 재사용, 실시간 단계별 로그) | `backend/app/routers/servicenow.py` `POST /servicenow/register/{item_id}` · `GET/PUT /servicenow/config`(`AppSetting` key=`servicenow_integration`) · `services/servicenow_service.py` | `components/work-items/ServiceNowRegisterDialog.tsx` · `hooks/useServiceNow.ts` · `components/settings/ServiceNowIntegrationPanel.tsx` |
...
| K8S 자원 관리(req/lim/use 랭킹 + 노드/NS 클릭 → 리소스 할당·사용 상세) | `backend/app/routers/k8s_allocation.py`(`GET .../nodes/{node}/pods` 포함) · `services/snapshot_jobs.py`(Redis 공유 스냅샷) · `services/k8s_paging.py` → `frontend/src/pages/K8sAllocationPage.tsx` · `frontend/src/components/k8s-allocation/`(뷰 컴포넌트 · `SummaryStrip`(압축 요약) · `AllocDetailDialog`+`drilldown.tsx`/`podGroup.ts`(노드/NS 상세) · 정렬/포맷 헬퍼) · `hooks/useK8sAllocation.ts` |
| K8S 자원 효율화(효율화 탭 — 히스토리 수집·request 축소 추천·적용/롤백·NS 정책·Quota 탄력·CR 어댑터) | `backend/app/routers/k8s_efficiency.py` · `models/k8s_efficiency.py` · `services/k8s_efficiency/`(`collector`·`engine`·`quota`·`apply`·`automation`·`history`·`settings`·`prometheus`·`owners`·`runs`) · `celery_app.py`(`dispatch_k8s_efficiency_collect`/`collect_k8s_efficiency_one`/`run_k8s_efficiency_run`) → `frontend/src/components/k8s-allocation/{EfficiencyTab,RecommendationTable,ApplyDialog,EfficiencyRunLog,EfficiencyRunPanel,NsTrendChart,LowEfficiencyRankingChart,PolicyDialog}.tsx` · `hooks/useK8sEfficiency.ts` |
...
| ↑ 두 SSH 콘솔 공용 base 툴 | `backend/app/services/ssh_pty.py`(WS↔paramiko PTY 브리지·init 프레임·토큰 검증) + `ssh_runner.py`(TOFU 연결) → `frontend/src/components/k8s/SshTerminalWindow.tsx`(xterm 창) · `frontend/src/lib/terminalPopout.ts`(창 간 handoff) · `frontend/src/lib/terminalClipboard.ts`(Ctrl+C 복사/Ctrl+V 붙여넣기 — PodTerminal 도 사용) · `hooks/useTerminalAppearance.ts`(`useXtermTheme`) |
| K8S 접근 권한(SA·Role·ClusterRole·Binding 발급/편집 + kubeconfig) | `backend/app/routers/k8s_rbac.py` · `services/k8s_rbac_service.py`(`RbacService` — CRUD·TokenRequest·kubeconfig 조립·SubjectAccessReview·단계 로그를 yield 하는 `provision()`) · `services/k8s_rbac_presets.py`(프리셋·바인딩 방식) · `schemas/k8s_rbac.py` → `frontend/src/pages/K8sRbacPage.tsx` · `frontend/src/components/k8s-rbac/` · `hooks/useK8sRbac.ts`(조회 훅 + SSE `useProvisionStream`) |
...
| Batch Jobs (cron) | `backend/app/routers/batch_jobs.py` + `services/batch_jobs/`(SSH: `etcdctl_defrag`/`shell_command`, non-SSH kubectl: `k8s_job_cleanup`, 스크립트 라이브러리 연동: `script_executor.py` — `job_type="script"`, `BatchJob.execution_mode="script"` 잡 전용, 실제 내용은 `execute_job()` 이 `script_id`/`script_version_id` 로 로드해 `ExecutionContext` 에 주입; base.py 에 단계 trace `_step`/`_record_command`/`step_plan`) + `services/k8s_diagnose.py`(kubectl 실패 분류·연결 원인 힌트) → `frontend/src/pages/BatchJobsPage.tsx`(독립 라우트 아님 — `HomePage.tsx` 의 "플랫폼 현황" 탭 → "배치잡" 서브탭이 임베드, 구 `/batch-jobs` 는 `/` 리다이렉트). "새 잡" 위저드(`CreateBatchJobWizard.StepType.tsx`)에 "실행 방식: 시스템 제공/스크립트 선택" 토글 — 스크립트 선택 시 kind=shell/ansible_playbook 만 가능(python 은 Phase 2 K8s Job 실행기 대기), 버전 고정(항상 최신/특정 버전) 선택 |
| Ansible 자산 (파일/인벤토리) | `backend/app/routers/ansible_assets.py` |
| 스크립트 라이브러리 (DB 저장·버전관리 Python/Ansible/Shell) | `backend/app/routers/scripts.py` + `models/executable_script.py`(`ExecutableScript`/`ExecutableScriptVersion` — `current_version_id` 순환 FK 는 `use_alter=True`) + `services/script_test_run.py`(test-run: shell→`ssh_runner`, ansible→`playbook_executor.run_playbook`, python→501 Phase 2 예정) → `frontend/src/pages/ScriptsPage.tsx` + `components/scripts/` + `hooks/useScripts.ts`. Phase 2 — `BatchJob.script_id`/`script_version_id` 가 이 스크립트를 참조하기 시작해 `DELETE /scripts/{id}` 가 참조 중이면 409(`used_by_count`). 설계: `docs/02-design/features/batch-jobs-execution-redesign.design.md` |
...
| leaf 단위 opt-in 앱 카탈로그(사이드바+상단바, Main UI 간소화 + 후속 개편) | `frontend/src/components/layout/installableApps.ts`(leaf 단위 설치 가능 카탈로그 + `sidebarAppSections()`/`topbarAppSections()`) + `AddAppDialog.tsx`(공용 카드형 추가 다이얼로그) → `HomePrefs.installedApps`(`backend/app/schemas/home_prefs.py`, 기본값 `["/tasks-mgmt"]`, `home_prefs.py` 라우터 재사용) — 기존 계정 이관은 `backend/app/main.py::_backfill_installed_sidebar_apps()`(1단계, 그룹 단위) + `_migrate_installed_apps_to_leaf_paths()`(2단계, leaf 치환 + 업무 도메인 grandfather) + `_prune_topbar_apps_to_default()`(3단계, 상단바를 `/tasks-mgmt` 하나로 정리) |
| 전역 상단바(업무 도메인) | `frontend/src/components/layout/AppTopBar.tsx` + `NavFlyout.tsx`(`FlyoutShell`/`FlyoutLink`/`FlyoutAction` 공용, Sidebar 와 공유 — 키보드 메뉴 규약은 `DESIGN_SYSTEM.md` §12.8) — opt-in 설치된 업무 leaf + 즐겨찾기/Your Island, `lg:` 미만(<1024px)은 `Menu` 트리거 하나로 접힘(D-076, §12.10) |
| 지원 뷰포트 배너(D-076) | `frontend/src/components/layout/ViewportSupportBanner.tsx` → `PageStyleProvider.tsx` 최상단 전역 마운트, `DESIGN_SYSTEM.md` §12.10 |
