...
| leaf 단위 opt-in 앱 카탈로그(사이드바+상단바, Main UI 간소화 + 후속 개편) | `frontend/src/components/layout/installableApps.ts`(leaf 단위 설치 가능 카탈로그 + `sidebarAppSections()`/`topbarAppSections()`) + `AddAppDialog.tsx`(공용 카드형 추가 다이얼로그) → `HomePrefs.installedApps`(`backend/app/schemas/home_prefs.py`, `home_prefs.py` 라우터 재사용) — 기존 계정 이관은 `backend/app/main.py::_backfill_installed_sidebar_apps()`(1단계, 그룹 단위) + `_migrate_installed_apps_to_leaf_paths()`(2단계, leaf 치환 + 업무 도메인 grandfather) |
