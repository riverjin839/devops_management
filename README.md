...
| **인프라/스토리지** | 인프라 물리 토폴로지, 노드 서버스펙 자산 대장, Isilon NFS 모니터링, 서비스 토폴로지, LAKE/PEP/APP 서비스 카탈로그 |
| **협업/지식** | 업무 보드(칸반/표/캘린더), 스프린트, Jira 양방향 연동(Excel 가져오기·상태/내용 반영), 주간 타임라인, WBS, 워크플로우, 마인드맵, 온톨로지 그래프, 지식 허브(문서 에디터·도구 탭 통합), 기술 트렌드 다이제스트 |
