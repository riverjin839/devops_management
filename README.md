...
| **리소스 탐색(읽기전용)** | K8s 리소스 11종 조회 + YAML 보기(Secret 마스킹), 파드 로그 스트리밍, 실시간 이벤트 |
| **자동화** | Ansible 플레이북·배치(SSH), etcdctl/mc 콘솔, 노드 일괄 실행 |
| **설정 변경 이력** | 스냅샷(해시 dedup) + diff + 감사 로그 |
| **협업/지식** | 업무 보드(칸반/표/캘린더, 시간옵션·이번주 필터), 주간 타임라인, WBS, 워크플로우, 마인드맵, 지식 허브(문서 에디터·도구 탭 통합) |
| **배포/운영** | kind / Helm / Kustomize / ArgoCD / Jenkins, 폐쇄망(airgap) 배포, JSON 백업·복원, RBAC |
...
- 배포: [배포 가이드](docs/DEPLOY_GUIDE.md) · [Mac 로컬](docs/MAC_LOCAL_TEST_GUIDE.md) · [Windows 로컬](docs/WIN_LOCAL_TEST_GUIDE.md) · [폐쇄망 LLM/Nexus](docs/AIRGAP_LLM_NEXUS.md)
