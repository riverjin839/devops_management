...
| D-026 | 접근성/일관성 | 자체 모달 raw `fixed inset-0`+`bg-black/40`+`rounded-2xl` (25파일/67곳) — Escape/포커스트랩/aria 부재, 테마 미적용 | 키보드로 모달 열고닫기 불가, 배경 포커스 누수 | 중간 | 부분완료 | **재사용 접근성 훅 `useModalA11y`(Escape·포커스 트랩·초점 이동/복원) 신설** + 공용 `ConfirmDialog`(앱 전역 확인 다이얼로그) 에 적용(role=dialog·aria-modal·aria-labelledby, main 과 독립 토큰화 병합)·운영점검 상세·일일점검 Schedule/Items 모달에 적용. **추가 확산분: 폼/다이얼로그형 전용 모달 8종(AddLakeServiceModal·AddServiceInstanceModal·ProjectFormModal·ClusterUpdateDiffDialog·ImageDistributeDialog·PlaybookLogDialog·CreateBatchJobWizard·BatchJobSlideOver overlay)에 `useModalA11y` 부착(수동 Escape effect 제거·role=dialog/aria-modal/aria-labelledby 부여). 남은 `fixed inset-0` 중 다수는 드롭다운/팝오버 click-catcher·터미널·캔버스로 모달 아님(트랩 미대상). 잔여 페이지형 모달(Sprints·Users·InfraTopology·Settings·NodeSpec·MindMap)·SidePane 은 다음 라운드 — 점진 확산 |
...
| R-4 화면 단위 UX 리뷰 | docs/SCREENS.md 기준 주요 화면 순회 정성 리뷰 | 감사 1회차 완료 ✅ | 진행중 — 1차(홈·대시보드·운영점검·업무, D-011~D-019) 처리 완료(D-018 모달→Dialog 리팩터만 보류), 2차(K8s상세/자원관리·일일점검·LAKE, D-020~D-028) 완료(D-026 모달 확산·D-027 skeleton 잔여). 다음 라운드 후보: 인프라 토폴로지·서비스 토폴로지·Pod 병목·Deep Check 정의 관리 등 |
