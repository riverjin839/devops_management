...
| D-048 | 안전/위험동작 | **되돌릴 수 없는 동작이 표준 게이트를 안 거침 (D-022·D-014 미적용)** — ① 클러스터 삭제가 브라우저 native `confirm()`(281)인데 실제로는 Addon·Playbook·점검 이력까지 캐스케이드 삭제 ② "노드 IP 일괄 수집"(339-359)은 확인 없이 **모든 미수집 클러스터에 dryRun 없는 auto-update 를 순차 적용**(이름과 달리 hostname/CIDR/버전까지 갱신), 취소 불가·진행률 없음, `fail` 이 전건이어도 `toast.success('일괄 수집 종료')` | 오조작 시 이력까지 소실되고, 일괄 수집은 무엇이 얼마나 바뀌는지 모른 채 N개 클러스터를 변경 | 높음 | 미처리 | 삭제→`ConfirmDialog`(danger, 대상·연쇄 삭제 범위 명시), 일괄 수집→대상 수 확인 다이얼로그+진행률(n/N)+중단 버튼, 실패 시 `toast.error`/부분 실패 구분 |
...
| ID | 영역 | 문제 | 사용자 영향 | 심각도 | 상태 | 비고 |
|---|---|---|---|---|---|---|
| D-054 | 정보구조 | **홈 모드(work/platform)가 사이드바 네비게이션 전체를 게이팅** — `Sidebar.visibleGroups`(267-270)가 `GROUPS.modes` 로 필터해, work 모드에선 클러스터/서버/네트워크/스토리지/DevOps 그룹(36개 화면)이, platform 모드에선 협업 그룹(7개 화면)이 레일에서 완전히 사라짐. 반대 모드 기능은 "존재한다는 단서(정보 냄새)" 자체가 0 | 임의 화면에서 반대 도메인 화면 진입이 최소 4클릭(로고→홈, 로고→토글, 레일, flyout). 신규 사용자는 "메뉴가 없어졌다"고 오인 — 사용자 제보의 직접 원인 | 높음 | 미처리 | 개선안 B(사이드바 통합+홈 탭) 권장 — 아래 로드맵 |
| D-055 | 발견성/일관성 | **모드 전환 수단이 로고 버튼의 숨은 토글 하나뿐 + 위치 의존 이중 동작** — `handleHomeClick`(192-199): 홈에서는 모드 토글, 다른 화면에서는 홈 이동. 라벨 없는 아이콘 버튼 하나가 브랜드/홈/모드전환 3역할. 전환 존재는 hover 툴팁으로만 학습 가능 | 같은 버튼이 상황마다 다르게 동작(일관성 위반)해 모드 전환을 발견 못 하거나, 홈 가려다 의도치 않게 화면이 통째로 바뀜 | 높음 | 미처리 | 명시적 세그먼트 컨트롤(라벨 있는 "업무 현황/플랫폼 현황" 탭)로 대체 |
| D-056 | 상태 지속성 | **로그인마다 `setMode('work')` 강제 리셋** (`authStore.ts:66-68`) — localStorage 에 모드를 저장해 놓고 로그인 시 무효화 | 플랫폼 현황을 주 화면으로 쓰는 운영자가 매 세션 수동 재전환 — 개인 선호가 유지되지 않음 | 중간 | 미처리 | 리셋 제거(마지막 모드 복원) 또는 사용자 설정(기본 홈 모드)으로 승격 |
| D-057 | 정보구조 | **지식 도메인 라우트 8개가 사이드바 고아** — `/docs`(지식 허브)·`/work-guides`·`/ops-notes`·`/mindmap`·`/ontology`·`/trends`·`/k8s-logs`·`/jira-import` 가 NAV_MAP 에는 있으나 어떤 GROUPS 에도 미포함. 진입로가 URL 직접 입력·Your Island 등록·산발적 컨텍스트 링크뿐 | CLAUDE.md 가 정의한 4대 축(모니터링/인프라/협업/**지식**) 중 지식 축이 네비게이션에서 실종 — 클릭 수 측정 불가(도달 불가) | 높음 | 미처리 | "지식" 그룹 신설(양 모드 노출 또는 통합 레일) — `/k8s-logs` 는 클러스터 그룹 편입 |
| D-058 | 선택지 과부하 | **'클러스터' 그룹 flyout 이 20개 항목 단일 플랫 리스트** (`navConfig.ts:80`) — 모니터링(현황/추이/이벤트/알람)·콘솔(k9s/SSH/etcdctl/일괄실행)·점검(운영점검/리뷰/항목관리)·관리(클러스터 관리/버전) 가 구분 없이 나열. 반면 work 모드는 실질 그룹 1개(협업 7) — 모드 간 밀도 극단 불균형 | Hick's law — 20지선다 스캔으로 목표 항목 탐색 시간 증가, 매 진입마다 반복 | 중간 | 미처리 | flyout 내 섹션 구분선+소제목, 또는 그룹 분할(모니터링/운영콘솔/점검) |
| D-059 | 일관성 | **레일 버튼 동작 비일관 + 서비스 도메인 모드 분산** — ①paths 1개 그룹(PEP/APP 서비스, 서비스/앱)은 클릭 즉시 이동, 2개 이상은 flyout — 시각 구분 없음 ②서비스 관련 그룹이 '서비스/앱'(platform, /lake-services)과 'PEP/APP 서비스'(work)로 두 모드에 분산 | 어떤 아이콘이 열리고 어떤 것이 이동인지 예측 불가, 서비스 정보를 찾는 위치가 모드에 따라 달라짐 | 중간 | 미처리 | 직행 그룹에 시각 힌트(chevron 부재 등) 또는 전부 flyout 통일, 서비스 도메인 그룹 단일화(D-030/031 연장) |
| D-060 | 첫 화면 가시성 | **랜딩(work 모드) 위 폴드에 플랫폼 상태가 KPI 필 1개뿐** — 로그인 직후(work 강제) 클러스터 건강 정보는 "위험 클러스터 N/M" 필 하나. 점검 매트릭스 요약·최근 실패 항목 없음. 역으로 platform 모드에선 업무 정보가 KPI 필뿐. 즐겨찾기/최근 방문 진입 수단도 부재(Your Island 는 수동 구성 필요) | 운영 도구인데 첫 화면에서 "지금 플랫폼이 괜찮은가"를 확 인지할 수 없음 — 모드를 오가며 확인해야 함 | 중간 | 미처리 | 홈 통합(개선안 A/B) 시 자연 해소. 단기: work 홈에 플랫폼 요약 스트립(클러스터별 상태 점) 추가 |
...
| R-4 화면 단위 UX 리뷰 | docs/SCREENS.md 기준 주요 화면 순회 정성 리뷰 | 감사 1회차 완료 ✅ | 진행중 — 1차(홈·대시보드·운영점검·업무, D-011~D-019) 처리 완료(D-018 모달→Dialog 리팩터만 보류), 2차(K8s상세/자원관리·일일점검·LAKE, D-020~D-028) 완료. 3차(클러스터 정보 수정, D-031~D-040) 완료. **4차(클러스터 관리 목록 `/cluster-manage`, D-041~D-053) 발견 등재 — 처리 대기(높음 5건: D-041·D-042·D-043·D-044·D-048·D-049)**. **5차(홈 모드 분기·네비게이션 IA, D-054~D-060) 발견 등재 — 개선안 확정 대기(권장: 사이드바 모드 게이팅 해제 + 홈 탭 전환, 단계 2로 지식 그룹 신설·클러스터 그룹 분할)**. 다음 라운드 후보: 인프라 토폴로지·서비스 토폴로지·Pod 병목·Deep Check 정의 관리 등 |
