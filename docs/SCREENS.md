...
- **파일**: `frontend/src/pages/WorkItemBoardPage.tsx` (+ `components/work-items/WorkItemKanban.tsx`, `WorkItemCalendar.tsx`, `WorkItemTableRow.tsx`, `WorkItemEpicView.tsx`, `WorkItemActionsMenu.tsx`, `ColumnSettingsMenu.tsx`, `JiraImportModal.tsx`, `JiraProvisionModal.tsx`, `JiraLinkDialog.tsx`, `JiraIssueChip.tsx`, `DocLinkChip.tsx`, `components/dashboard/QuickAddTaskModal.tsx`)
...
- **UI 구성**:
  - 헤더: 전체/WIP/Done 카운트 배지, 뷰 전환(`ViewModeBar`: 목록/달력/칸반/에픽뷰), "JIRA" 버튼(Jira 가져오기, `jiraConfig.enabled`일 때만) · "Confluence" 버튼(Confluence 연동, `jiraConfig.confluenceBaseUrl` 설정 시만) · "CSV" 버튼(CSV 추출) — 셋 다 버튼 라벨은 짧게 줄이고 전체 설명은 `title` 툴팁으로 옮겼다 · 업무 등록 버튼
...
- **Backend**: `GET /api/v1/work-items`(목록, 필터 쿼리파라미터 snake_case 변환 — `jira_issue_type`(등록 타입, ILIKE 정확 일치)도 포함), `POST /api/v1/work-items`, `DELETE /api/v1/work-items/{id}`, `GET /api/v1/work-items/export/csv` — `backend/app/routers/work_items.py`. 프로젝트명/스프린트명 매핑을 위해 `GET /api/v1/projects`(`projects.py`), `GET /api/v1/sprints`(`sprint.py`)도 호출. Jira 가져오기 가능 여부는 `GET /api/v1/jira/config`(`jira.py`). 행 단위 동기화는 `POST /api/v1/jira/refresh/{id}` · `POST /api/v1/jira/push/{id}`, 연계 생성은 `GET /api/v1/jira/provision/defaults` · `POST /api/v1/jira/provision`. Confluence 상위 페이지 탐색은 `GET /api/v1/jira/confluence/page-info`(제목 조회) · `GET /api/v1/jira/confluence/children`(하위 페이지 목록). 연결 복구는 `POST /api/v1/jira/unlink/{id}` · `POST /api/v1/jira/relink/{id}` · `POST /api/v1/jira/verify-links`. 모델: `backend/app/models/work_item.py` (WorkItem), `backend/app/models/user_jira_credential.py`(`jira_username` — 담당자 매핑용 원본 Jira 로그인 계정). **Jira 가져오기 제목**: `map_jira_issue`(`services/jira_service.py`)와 엑셀 저장(`import_excel_save`) 모두 `title` 을 요약문만 저장한다(구 `"{키} {요약}"` 접두어 형식 제거 — 이슈 키는 화면에서 DL# 컬럼이 별도로 보여줌). Jira 로 되돌려 보낼 때 접두어를 떼는 `strip_issue_key_prefix` 는 과거 데이터 호환용으로 유지.
- **핵심 기능**:
