...
- **UI 구성**:
  - KPI 스트립(탭과 무관하게 항상 노출): **Your Island 진입 필**(`IslandPill` — 마지막에 보던 아일랜드로 이동, 없으면 "만들기"), 이어서 KPI 필 4종(`내 할일`→`/todo-today`, `미해결 이슈`→`/tasks-mgmt`, `위험 클러스터`→`/cluster-overview`, `다음 일정`→`/tasks-mgmt`).
