...
- **핵심 기능**:
  ...
  - **행별 즉시 실행**: 잡을 열지 않고 테이블에서 바로 실행하는 ▶ 아이콘(저장 자격증명/non-SSH 필요, 없으면 비활성화+안내)
