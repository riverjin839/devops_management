...
- **UI 구성**:
  ...
  - **플랫폼 현황 탭**: 안에 다시 서브탭 2개 — **점검 매트릭스**(기본, `LayoutGrid` 아이콘) / **배치잡**(`ListTree` 아이콘, `useNavCatalog().featureAllowed('/batch-jobs')` 가 꺼지면 서브탭 버튼 자체가 숨음). 로컬 state(`platformView`)만 쓰고 서버/localStorage 영속은 없다 — 탭을 나갔다 오면 매트릭스로 리셋. **점검 매트릭스** 서브탭: `PlatformStatusMatrix` — 행(점검 항목) × 열(등록된 클러스터) 매트릭스. 항목이 늘어나도 admin 이 한눈에 훑을 수 있게 행을 **item.category 기준 그룹**(K8s/네트워크/스토리지/OS/애플리케이션 순, 미분류는 맨 끝)으로 묶고 그룹 헤더 행(colSpan 전체 너비, 클릭으로 접기/펼치기)에 항목 수 + 정상/경고/위험 카운트 배지를 상시 표시 — 기본은 전부 접힌 상태이되 위험/경고 셀이 하나라도 있는 그룹만 자동으로 펼쳐진다. 이 자동 판정은 사용자가 한 번이라도 직접 접기/펼치기를 건드리기 전까지만 적용되고, 그 이후엔 `localStorage`(`pep:checkMatrixCollapsedGroups`) 에 저장된 사용자 선택을 그대로 따른다(표시 설정 팝오버에 "모두 펼치기"/"모두 접기" 단축 버튼). 그룹 안에서 첫 열은 sticky(그립(⋮⋮) **드래그로 순서 변경** + 항목명 + **영역 칩·행 배경 색**(category/color — 8색 차트 토큰 프리셋, 항목 폼에서 지정) + 실행방식 배지 + 상시 ▶(전 클러스터 실행) + hover 시 수정/삭제 아이콘, 시스템 항목은 삭제 버튼 숨김), 클러스터 열 헤더는 이름 + cron 배지(클릭 시 팝오버로 `Cluster.check_cron_expr` 편집 — 배지 색은 **중지**(회색, cron 미설정)/**실행중**(파랑, 전역 활성 수행 폴링)/**정상·경고·위험**(초록/주황/빨강, `core_bundle` 행의 최근 셀 상태)으로 한눈에 판독 가능), 셀은 상태 dot + 값/라벨, hover 시 우상단에 셀 단독 즉시실행 ▶(수동 입력 항목 제외 — 클릭 시 `ConfirmDialog` 확인 후 `useRunCheckMatrixCell` 로 그 셀만 실행, 이벤트 버블링 차단으로 상세 모달은 안 열림)(클릭 시 `CheckMatrixCellDetailModal` — 탭 3종: **추이 · 이력**(기간별 트렌드 차트 + 변경 이력 + manual 항목이면 값 입력 폼 + core_bundle 이외 항목이면 항목×클러스터 cron 편집) / **실행 방식**(`CheckMatrixRunbookPanel` — 이 셀의 가장 최근 수행을 짧은 주기로 폴링해 계획 대신 실제 결과로 단계를 색칠하고 진행 중이면 상태 배지로 보여줌 — 폴링 중(running)에도 백엔드가 채워 보내는 단계별 상태를 그대로 넘겨 실행 중에도 실시간으로 색이 입혀진다. 스크롤을 줄이려 이 탭에서만 모달이 `max-w-4xl` 로 넓어지고 실행 대상/최근 수행 상태를 한 줄에, 수행 명령/설정값(또는 편집 폼)·메모를 2열로 배치하며 명령·설정값 목록은 각자 내부 스크롤(`max-h-72`)로 캡핑된다) / **수행 로그**(`CheckMatrixRunList`+`CheckMatrixRunDetailView` — 목록·상세 모두 실행 중인 수행은 자동 폴링, 완료(위험)/실패는 사유 콜아웃 + 결과 상세 자동 펼침), 헤더에 셀 단독 "지금 실행" 버튼). 실행 단계 타임라인(`ExecutionStepsTimeline`, `components/daily-check/`)의 각 단계 아이콘은 클릭하면 그 단계의 상세 로그(전체 텍스트·metrics·소요시간)를 펼쳐 보여준다. 매트릭스 영역은 페이지 남는 세로 공간을 모두 채우고(고정 높이 제한 없이 `flex-1`), 카드 안쪽 스크롤 하나만 갖는다(이중 스크롤 없음). 클러스터 열 헤더에 **열 전체 실행 ▶**, 항목 행 hover 에 **전 클러스터 실행 ▶** 버튼. 카드 헤더에 "수행 로그"(`CheckMatrixRunLogPanel` — 전체/배치 필터, 배치 추적 시 3초 폴링) + "항목 추가"(`CheckMatrixItemFormModal`) + 도움말 `?`(`CheckMatrixHelpPanel` — 기본 사용법/실행하기/점검 방식/로그·보관 4탭 매뉴얼) + 설정 톱니바퀴(`CheckMatrixSettingsModal`, 이력 보관 일수) 버튼. **배치잡** 서브탭: `BatchJobsPage` 를 그대로 임베드 — 화면 구성/기능은 아래 "Batch Jobs" 섹션 참고.
  - ClusterSidebar 미사용(홈은 특정 클러스터에 종속되지 않음).
- **Frontend**: `useHomeStore`(Zustand, `homeTab`/`scheduleBg`, localStorage 키 `pep:homeTab`/`pep:scheduleBg` — 구 `pep:homeMode` 키는 1회 마이그레이션) · `useClusterStore` + `useClusters()`(TanStack Query — 플랫폼 탭 배지의 위험 클러스터 수만 파생, KPI 스트립 제거 후에도 다른 화면과 clusterStore 를 공유하기 위해 호출은 유지) · 플랫폼 탭 배지의 `점검 실패` 는 `useCheckMatrixFailureCount()`(파생 전용, `select` 로 grid 를 critical 셀 개수로 변환) · 플랫폼 탭은 `hooks/useCheckMatrix.ts`(`useCheckMatrixGrid`/`useCheckMatrixItems`/`useReorderCheckMatrixItems`/`useDeleteCheckMatrixItem`/`usePutClusterCron`/`usePutSchedule`/`usePostManualEntry`/`useCheckMatrixCellHistory`/`useCheckMatrixSettings`/`useCheckMatrixRunbook`/`useRunCheckMatrixCell`/`useRunCheckMatrixCluster`/`useRunCheckMatrixItem`/`useCheckMatrixRuns`/`useCheckMatrixRun`) · **기본 홈 탭**은 `hooks/useHomePrefs.ts`(`useHomePrefs`/`useUpdateHomePrefs`) — 로그인 후 최초 1회 `homePrefs.defaultHomeTab` 을 `useHomeStore.setHomeTab()` 에 반영(`appliedServerDefault` ref 로 재적용 방지)하고, 이후 탭을 바꾸면 500ms 디바운스로 서버에 저장. 로컬 state: `weeklyTab`.
- **Backend**: `GET /api/v1/clusters`(`clusters.py`) · `GET /api/v1/work-items`(`work_items.py`) · `GET/PUT /api/v1/me/home-prefs`(`home_prefs.py` — `defaultHomeTab`/`pinnedPaths`, `user_settings` 테이블 재사용) · 플랫폼 탭은 `check_matrix.py` 라우터(prefix `/check-matrix`): `GET /items`, `POST/PUT/DELETE /items(/{id})`, `POST /items/reorder`, `GET /grid`, `GET /cell/{item_id}/{cluster_id}/history`, `POST /cell/{item_id}/{cluster_id}/manual-entry`, `PUT /schedule/{item_id}/{cluster_id}`, `PUT /clusters/{cluster_id}/cron`, `GET/PUT /settings`, `GET /exec-techs`(등록 마법사 카탈로그 + `creatable` 직접 만들기 선택지), `POST /items/preview`(저장 전 1회 실행 — 플레이북 본문 직접 실행 시 `check_mode` dry-run + `log` 반환), `GET/PUT /items/{item_id}/source-config`(행 단위 임계값·파라미터, `dedicated` 로 행 전용 정의 분리), `GET /cell/{item_id}/{cluster_id}/runbook`, `POST /cell/{item_id}/{cluster_id}/run`(동기), `POST /clusters/{cluster_id}/run`·`POST /items/{item_id}/run`(Celery fan-out → batch_id), `GET /runs`·`GET /runs/{run_id}`. 실행/집계는 `services/check_matrix_service.py`(`build_grid`/`get_cell_history`/`dispatch_due`/`purge_expired_logs`/`seed_default_items`/`create_run`/`execute_run`/`run_cell_now`/`run_cluster_now`/`run_item_now`/`list_runs`/`get_run`/`resolve_definition_for_item`/`create_dedicated_definition`/`create_playbook_card_targets`), 실행 계획(런북)은 `services/check_matrix_runbook.py`.
- **핵심 기능**:
  - 업무 현황/플랫폼 현황 세그먼트 탭 전환(localStorage 영속, **로그인해도 리셋되지 않음** — 예전엔 `authStore.setSession()` 이 로그인마다 강제로 `work` 로 되돌렸다, R-4 5차 D-056). 기본 홈 탭은 `SettingsPage`의 "화면 UI 설정" 탭에서 지정하면 다음 로그인부터 어느 기기에서든 해당 탭으로 열린다.
  - 담당자별 진행 현황을 주간/월간/담당자 3가지 뷰로 전환, 업무 탭 스케줄 배경색(흰색/크림) 커스터마이즈(`SettingsPage`의 화면 UI 설정 탭에서 변경).
  - 플랫폼 현황 탭: 클러스터 상태에 영향을 주는 핵심 점검(API 서버 응답시간, `is_system=true`, 삭제 불가)은 클러스터 열의 cron(`Cluster.check_cron_expr`)으로, 그 외 자동 점검(deep_check/addon 소스)과 수동 항목은 항목×클러스터별 cron(`CheckMatrixSchedule`, 5분 미만 간격 거부)으로 독립 스케줄. `Cluster.status`는 `DailyChecker.run_daily_check()`(무변경)가 여전히 유일한 authoritative 소스이며 `check-matrix-dispatch` Celery Beat(매분)가 due 한 클러스터에 대해 그대로 호출한다.
  - 셀 이력은 `CheckMatrixResultLog`에 append-only 저장되고, 설정한 보관 일수를 초과하면 `check-matrix-log-purge` Beat(매일 03:00 KST)가 청크 삭제로 정리한다.
  - **수행 로그**는 별도 `CheckMatrixRun`(trigger=cron|manual_cell|manual_cluster|manual_item|manual_entry, run_state=queued→running→success/failed/skipped, batch_id, triggered_by, details 에 `_steps`/`_commands`/`_runbook`)에 쌓인다 — 판정 없는 수행(대기열·건너뜀)이 추이 차트를 오염시키지 않도록 값 이력과 분리했다. cron 자동 실행도 같은 표에 남고, 보관 일수는 값 이력과 공유한다. 실제 나간 kubectl 명령은 `DeepCheckerBase._kubectl` 이 수집해 `_commands` 로 보존.
...
- **요청사항 (수정 요청)**:
  ...
  - **[2026-09-08, D-061~D-065]** 이 화면(플랫폼 현황 탭)을 로그인 직후 첫 화면으로: 홈 기본 탭 `work`→`platform` 전환 + 네비게이션에 진입 경로 추가. 행의 소스 배지("핵심/Deep/Addon/수동")를 실행 기술(K8s API/kubectl/HTTP/PromQL/스냅샷/SSH bash·python/Ansible) × 컴포넌트(기존 category) 2축으로 재명명. 클러스터 열 헤더·행 이름에 드릴다운 링크 추가(`/clusters/:id`, `/checks/:itemId` 신설). 항목 추가를 "실행기술→종류→컴포넌트→값→테스트(필수)→적용" 마법사로 재구성. 설계안: [점검 체계 통합 설계안](https://claude.ai/code/artifact/9d204e4a-c106-48e8-91b8-4d0b37ebc250)
    ...
    - **[2026-09-17]** 등록 마법사가 "정해진 카드"에 갇히지 않는다 — 종류 단계 아래 **직접 만들기** 구역(서버가 `GET /check-matrix/exec-techs` 의 `creatable` 로 알려주므로 프론트 하드코딩 없음)에서 ① **새 플레이북 만들어 등록**(Ansible): `플레이북 작성` 단계가 추가돼 이름·YAML(직접 작성 / `.yml` 파일 업로드 / 플레이북 라이브러리에서 불러오기)·태그·extra vars·실행할 클러스터(다중)를 지정하면 저장 시 `AnsiblePlaybookFile` + 클러스터별 `Playbook` + 매트릭스 행이 한 번에 만들어진다 ② **커스텀 점검(HTTP/kubectl/PromQL) 새 인스턴스**: 같은 점검 종류로 대상만 다른 카드를 여러 장 만들 수 있다. 값 설정 단계의 임계값/파라미터는 **기본 등록 종류에서도 더 이상 잠기지 않고**(예전엔 `seedDefault` 면 readonly), `이 항목 전용 설정으로 저장` 스위치로 행 전용 정의(`CheckMatrixItem.definitionId`)를 만들지 공유할지 고른다. 테스트 단계는 결과 아래 **로그 보기**(`LogViewer`) 로 상세 실행 로그를 펼쳐 주고, 새 플레이북은 기본 `ansible --check`(dry-run)로 돌린 뒤 체크박스를 꺼야 실제 실행된다. 항목 수정 폼(`EditItemForm`)에도 실행 설정 섹션이 생겨 기본 등록 카드의 임계값/파라미터를 바로 확인·수정하고 `이 항목 전용 설정으로 분리` 할 수 있다(`GET|PUT /check-matrix/items/{id}/source-config`).
...
- **UI 구성**:
  ...
  - **세션 만료 안내 배너**(D-079, `role="status"`): `authStore.logoutReason === 'expired'` 일 때 폼 위에 "세션이 만료되어 로그아웃되었습니다. 다시 로그인하면 보던 화면으로 돌아갑니다" 표시. 수동 로그아웃(`'manual'`)에는 없음.
...
- **핵심 기능**:
  ...
  - **세션 만료 흐름(D-079)**: axios 401 → `expireAuthSession()`(보던 `pathname+search` 를 `returnTo` 로 기록 + 사유 `'expired'`) / `hooks/useSessionExpiryWatch`(AppShell 에서 JWT `exp` 를 읽어 만료 5분 전 경고 토스트, 만료 시각 지나면 401 을 기다리지 않고 같은 처리). 재로그인 성공 시 `consumeReturnTo()` 로 복귀 경로를 소비해 현재 URL 과 다르면 `navigate(returnTo)`.
  - 로그인 성공 시 홈 탭을 강제로 되돌리지 않는다 — 마지막으로 보던 탭(또는 Settings 에서 지정한 기본 홈 탭)이 로그인 후에도 유지된다.
...
- **UI 구성**:
  ...
  - `연동 (ServiceNow)` 탭(`ServiceNowIntegrationPanel`): Base URL·테이블명·필드 매핑/우선순위 매핑(JSON, 실제 인스턴스 스펙 확인 전까지 표준 Table API 가정값을 코드 수정 없이 조정)·연동 활성화·TLS 검증 + 연결 테스트. 1차 구현은 전용 인증 UI가 없어 현재 로그인 사용자의 Jira/SSO 세션을 그대로 재사용한다는 안내만 표시한다(전용 인증은 추후 개선). 업무 목록/상세의 "ServiceNow ITSM 등록" 버튼(`ServiceNowRegisterDialog.tsx`, Jira 연동 업무만 대상)이 이 설정을 사용한다.
  ...
  - 나머지 탭은 각각 전용 매니저 컴포넌트를 그대로 렌더(운영레벨/시스템 담당자/접근제어/Jira/백업/감사로그).
...
- **Backend**: `GET/PUT/DELETE /api/v1/clusters`, `POST /api/v1/clusters/{id}/verify`(`clusters.py`) · `GET/POST/PUT/DELETE /api/v1/management-servers`, ping 엔드포인트(`management_servers.py`) · `GET/PATCH /api/v1/ui-settings`(홈 아이콘 등, `ui_settings.py`, `AppSetting` 모델 기반 key-value 저장: `UI_SETTINGS_KEY`/`OPERATION_LEVELS_KEY`/`FEATURE_ACCESS_KEY`) · `GET/PUT /api/v1/me/home-prefs`(기본 홈 탭, `home_prefs.py` — 사용자별 `user_settings` 행) · **시스템 담당자**(명부+계정 통합): `GET/PUT /api/v1/ui-settings/assignees`(admin 전체 목록 upsert, `users` 테이블 직접 조회/갱신 — 삭제는 없음), `PUT /api/v1/ui-settings/assignees/me`(본인 행 부분 수정, 로그인 사용자 누구나) — `ui_settings.py`. 계정 역할 변경/비밀번호 재설정/삭제/명부 무관 계정 생성은 `GET/POST /api/v1/auth/users` · `PUT /api/v1/auth/users/{id}/role` · `POST /api/v1/auth/users/{id}/password` · `DELETE /api/v1/auth/users/{id}`(`auth.py`, `Depends(require_admin)`, `User` 모델 role: `admin`/`operator`/`viewer`. 자기 자신 대상 삭제/역할변경은 400 거부, 모든 변경은 `audit_logger.record(action="user.create"/"user.delete"/"user.role.update")`) · 운영레벨/서비스카탈로그/Jira/백업/감사로그는 각 하위 매니저 컴포넌트가 별도 라우터(`work_item_custom_fields.py`, `jira.py`, `backup.py`, `audit_logs.py` 등) 호출 · 스키마 점검은 `GET /api/v1/schema-health`, `POST /api/v1/schema-health/repair?dry_run=`(`schema_health.py` + `services/schema_health.py`, admin 전용). 부팅 시에도 `main.py` 의 `_sync_missing_model_columns`(누락 컬럼 보강)와 `_relax_not_null_drift`(레거시 NOT NULL 완화)가 자동 실행된다.
- **핵심 기능**:
  ...
  - 시스템 담당자(명부+로그인 계정 통합 CRUD/역할변경/비밀번호 재설정, 본인 자기강등·자기삭제 이중 차단)/운영레벨/관리 서비스(PEP·APP 카테고리+서비스 타입)/접근 제어/Jira·Confluence 연동(SSO 자동 로그인 — Confluence URL 설정 시 한 번의 로그인으로 두 세션 동시 캡처, 만료 시 자동 재로그인)/Debug 로그/백업·복구/감사 로그 등 전역 운영 설정을 탭 단위로 통합 관리.
...
- **Backend**: `GET /api/v1/clusters`, `GET /api/v1/health/summary`, `GET /api/v1/health/addons/{clusterId}`, `POST /api/v1/health/check/{clusterId}`, `POST /api/v1/health/check/{clusterId}/addons/{addonId}`, `POST/PUT/DELETE /api/v1/health/addons...`, `GET /api/v1/health/report` — 라우터 `backend/app/routers/health.py` + `clusters.py`. 서비스: `services/core_bundle_checker.py`(`DailyChecker`), `services/health_checker.py`, `services/checkers/*`(argocd/control_plane/etcd/jenkins/keycloak/nexus/node/system_pod). 모델: `models/cluster.py`(`Cluster`, `StatusEnum`), `models/addon.py`(`Addon`), `models/metric_card.py`.
- **핵심 기능**:
...
- **목적 / UX**: Lens/OpenLens 스타일의 K8s 리소스 탐색기. 좌측 카테고리 내비(Cluster/Nodes/Namespaces/Workloads/Config/Network/Storage/Events/Helm/AccessControl/CRD 순 — Namespaces 는 Nodes 바로 아래)로 리소스 종류를 전환하며 목록 조회, YAML 상세/편집, scale/restart/delete/cordon/drain 같은 쓰기 작업, Pod 터미널·이벤트 확인까지 한 화면에서 처리. **Namespaces 카테고리는 "목록"(기존 Namespace 오브젝트 나열)과 "대시보드"(종류가 아니라 네임스페이스를 먼저 골라 그 안의 workload/config/network/storage 전체를 한 화면에서 보는 뷰) 두 리프로 나뉜다.**
- **UI 구성**:
  - `ClusterSidebar` — `iconOnly` 단일 선택(`allowAll` 없음), URL 파라미터(`:clusterId`) 동기화.
  - 두 번째 sticky 사이드바 — Lens 식 카테고리 내비(`NAV` 상수, `kindAvailability` 응답으로 존재하는 kind만 노출). `LeafMode`(`kind|overview|events|helm|crd|nsdash`)로 분기. **리프가 2개 이상인 대분류는 헤더를 눌러 접기/펼치기** 가능 — 상태는 `localStorage`(`pep:k8s:navCollapsed`)에 저장돼 다음 방문에도 유지되고, 현재 선택된 리프가 속한 대분류는 저장된 상태와 무관하게 항상 펼쳐 보인다.
  ...
  - `NamespaceDashboardPanel`(**대시보드** 리프) — `NamespaceMultiSelect`(`singleSelect` 모드, 종류별 탐색 화면과 동일한 "전체 네임스페이스" 버튼 + 검색 드롭다운)로 네임스페이스 1개 선택 → 엑셀 스타일 **리소스 인벤토리** 표(종류/구분(Workload·Config·Network·Storage 색 점)/개수/정상/경고/위험/비고, `caps[kind].namespaced` + `avail[kind].present`로 표시 대상 자동 필터, 종류별 `k8sResourcesApi.list`/`richPods` 병렬 조회로 개수·상태 산출 — 상태 개념이 없는 종류(ConfigMap 등)는 `—`) → 행 클릭 시 상세 목록으로 바로 이동(**탭 드릴인형**: 카테고리 탭 + 종류 필 로우 + 선택한 종류 패널이 먼저 나오고 인벤토리 표는 그 아래 / **전체 나열형**: 인벤토리 표가 위, 4개 카테고리 전부를 세로로 펼쳐 종류별 패널을 다 쌓은 목록이 아래, 상단 토글로 두 레이아웃 전환). 실제 리소스 테이블은 두 모드 다 같은 `ResourceTablePanel`/`PodsPanel` 인스턴스(`KindPanel` 래퍼로 NS 고정)를 쓰므로 스케일/재시작/삭제/YAML 편집·Pod 터미널이 종류별 탐색과 동일하게 동작. PV/StorageClass 등 cluster-scoped 종류는 제외.
  - `OverviewPanel`(노드/네임스페이스 통계), `HelmPanel`(릴리스 목록 + values 보기), `CrdPanel`(CRD 목록 → 오브젝트 드릴다운), `EventsStream`(events 탭).
  - `DetailDrawer` — 우측 슬라이드 패널(요약/YAML/이벤트 탭 — 이벤트 탭은 열려 있고 브라우저 탭이 보일 때만 30s 갱신), `RoleGate(['admin','operator'])`로 편집·쓰기 UI 게이팅, viewer는 읽기 전용 배지. 대시보드에서도 페이지 최상단의 같은 `DetailDrawer`/`PodTerminal`/`ConfirmDialog`를 콜백으로 그대로 재사용.
  - `PodTerminal` 모달(WebSocket exec).
- **Frontend**: 별도 커스텀 hook 없이 TanStack `useQuery`/`useQueries` 직접 사용(`k8s-mng-list`/`k8s-mng-nodes`/`k8s-mng-helm`/`k8s-mng-crds`/`k8s-mng-yaml`/`k8s-caps`/`k8s-avail` 등 쿼리키), `useColumnPrefs`(컬럼 표시/숨김 로컬 저장). `useClusters`. `NamespaceDashboardPanel`은 네임스페이스 목록을 `NamespaceMultiSelect`(내부적으로 `k8sResourcesApi.list(clusterId, 'namespaces')`) 경유로 얻는다. api.ts: `k8sResourcesApi.{list,yaml,resourceEvents,capabilities,kindAvailability,richNodes,richPods,scale,restart,remove,apply,cordon,drain,crds,crdObjects,crdObjectYaml}`, `k8sHelmApi.{releases,values}`.
- **Backend**: 라우터 `backend/app/routers/k8s_resources.py`(prefix `/k8s`) — `GET /k8s/{id}/resources/{kind}`(`namespace` 쿼리 파라미터 기지원 — 대시보드도 이 엔드포인트를 그대로 재사용, 백엔드 변경 없음), `GET .../yaml`, `GET .../events`, `POST .../scale`, `POST .../restart`, `DELETE .../{kind}/{ns}/{name}`, `PUT .../yaml`, `POST /k8s/{id}/nodes/{name}/cordon`, `POST /k8s/{id}/nodes/{name}/drain`, `GET /k8s/{id}/crds(...)`, `GET /k8s/{id}/resources-capabilities`, `GET /k8s/{id}/nodes`, `GET /k8s/{id}/pods`, `GET /k8s/{id}/kind-availability`; `backend/app/routers/k8s_helm.py`(`/k8s/{id}/helm/releases(...)`); Pod 터미널은 `backend/app/routers/k8s_exec.py`(`WS /k8s/{id}/exec`). kubernetes SDK 직접 호출(DB 캐시 없음) — 단 목록 3종(`resources/{kind}` · `nodes` · `pods`)과 상세 drawer 이벤트(`.../events`)는 **서버 목록 캐시**(`services/k8s_list_cache.py`, fresh 5s / stale 30s SWR + single-flight, apiserver 단위 그룹 — 탐색기뿐 아니라 효율화 적용·노드 라벨·RBAC·배치잡 kubectl 삭제 등 PEP 의 모든 쓰기가 모든 replica 의 캐시를 자동 무효화)를 거치고 응답을 원본 JSON 으로 파싱(`services/k8s_raw.py`, 모델 역직렬화 생략)한다. 응답에 `cache`(hit/stale/miss)·`cache_age_seconds` 포함. 패널의 **새로고침 버튼만** `refresh=true` 로 캐시를 건너뛴다. 파드 패널은 **200건 페이지 + 무한 스크롤**(`GET /k8s/{id}/pods?limit=200&continue=<토큰>`, 응답 `continue_token`/`remaining_count`, 토큰 만료는 410 → 새로고침으로 처음부터) — 검색은 불러온 파드 기준이며 푸터에 불러온 수/전체 추정치와 "다음 200개 불러오기" 버튼이 있다(네임스페이스 대시보드 임베드는 기존처럼 NS 전량 조회). 노드·네임스페이스 등 작은 cluster-scoped 목록은 `resource_version=0`(watch cache)으로 읽는다. 대상 apiserver 동시 호출 상한 대기 초과는 503 으로 사유를 표시한다. `kind-availability` 는 종류당 `limit=1` 프로브 + 5분 캐시.
- **핵심 기능**:
...
- **파일**: `frontend/src/pages/K8sAllocationPage.tsx`(페이지 셸 — 요약 스트립/진행 슬롯/sticky 탭/자동갱신/상세 다이얼로그 상태) + `frontend/src/components/k8s-allocation/`(`SummaryStrip`(압축 요약), `SummarySection`, `PodScheduleCalc`, `PodCapacityStatusCards`, `NodesView`, `NamespacesView`, `NsRankingView`, **`AllocDetailDialog`**(노드/NS 상세), 공용 `drilldown.tsx`(`WorkloadTable`/`WorkloadsDrill`/`PodsDrill`/`PodAllocTable`/`NodeWorkloadsDrill`)·`podGroup.ts`·`format.ts`/`primitives.tsx`/`tableSort.ts`), + `components/common/{EmptyState,Skeleton,SnapshotProgressCard,SnapshotProgressBar,ExportMenu}`.
...
- **UI 구성**:
  ...
  - 진행/안내 슬롯 — 집계 중엔 `SnapshotProgressBar`, 아니면 partial/stale 안내(표시할 게 없으면 높이를 차지하지 않는다).
  - **`SummaryStrip`(기본)** — 노드/NS/파드/배치여유/CPU·MEM 할당·사용효율/여유·낭비/Run·Pend·Err 를 **칩 한 줄**로 압축. 큰 요약 카드가 세로를 다 먹어 정작 봐야 할 자원 탭이 접히는 선 아래로 밀리던 문제를 없앤 것 — 탭이 화면 위쪽에 오고 스크롤이 최소화된다.
  - `요약 상세` 토글(localStorage `pep.k8s-allocation.summaryDetail`, **기본 접힘**) — 켜면 기존 `클러스터 요약` MacCard(할당·사용효율 스탯 + 물음표 툴팁, `PodScheduleCalc`, 할당 가용/추정 낭비 라인)와 `PodCapacityStatusCards`(**POD 용량** / **POD 상태**)가 스트립 아래로 펼쳐진다.
  - 4-탭 전환(`view`, **sticky** — 긴 표를 스크롤해도 탭 바가 화면에 남는다): `NodesView`(카드/테이블 뷰 토글, 정렬·검색·CSV. **48노드 초과 시 `react-virtuoso` 가상 스크롤**(카드 `VirtuosoGrid`, 테이블 `TableVirtuoso`, 뷰포트 64vh)), `NamespacesView`(네임스페이스 → 워크로드 → 파드 인라인 드릴다운, 페이징), `NsRankingView`(req vs 실사용 막대 랭킹 차트), **`EfficiencyTab`(효율화)**.
  - **`AllocDetailDialog`(기준 정보 클릭 → 리소스 상세)** — 세 탭이 공유하는 단일 다이얼로그. `NodesView` 의 노드명(카드/테이블), `NamespacesView` 의 NS 명, `NsRankingView` 의 막대·Y축 라벨을 클릭하면 열린다. 노드 스코프는 헤더에 CPU/MEM `MeterBar`(alloc 대비 req·use) + 워크로드/파드/NS/역할 스탯, 본문은 `NodeWorkloadsDrill`(그 노드에 뜬 파드를 Deployment/STS/DS/Pod 로 묶어 req·limit·usage 합산 → 펼치면 파드·컨테이너별 `PodAllocTable`). NS 스코프는 NS 스탯(파드/워크로드/CPU·MEM req·use·limit/req미설정) + `WorkloadTable` → `PodsDrill`. 양쪽 다 검색·CSV·개별 새로고침 제공.
  - **효율화 탭**(`components/k8s-allocation/EfficiencyTab.tsx`): 툴바(수집 cron/마지막 수집/자동 적용 ON·OFF/사용률 소스/절감 가능 합계 + **지금 수집** · **추천 재생성** · **정책 설정** 버튼) → `RecommendationTable`(컨테이너별 request 축소 추천, 다중 선택 → `ApplyDialog`(before/after, **dry-run 기본 on**, "실행 후 실시간 로그 보기"), 무시. 오퍼레이터(CR) 관리 워크로드는 "오퍼레이터 관리 — CR로 조정" 배지 + 적용 불가) → `NsTrendChart`(NS 별 request/실사용/Quota 한도 시계열 24h/7d/30d, CPU/MEM) + `LowEfficiencyRankingChart`(사용효율 낮은 NS 상위 8 추이 + 추세 화살표, 클릭 시 추이 차트 연동) → `EfficiencyRunPanel`(실행 이력, 행 펼치면 `EfficiencyRunLog`). **모든 실행 버튼은 run 을 만들고 `EfficiencyRunLog`(`ExecutionStepsTimeline` + `LogViewer`, 1.5초 폴링)를 즉시 띄운다 — "로그 보기" 토글(localStorage 기억)로 접을 수 있다.** 롤백 버튼은 비 dry-run 성공/부분 성공 run 에만. `PolicyDialog`(`useModalA11y`): 네임스페이스 정책(auto right-size/Quota 탄력 opt-in, Quota min/max — "라이브 Quota 불러오기" 프리필, 오버라이드, **오퍼레이터 CR 어댑터**(group/version/plural/name/jsonpath/min/max — 예: StarRocks CN `spec.starRocksCnSpec.replicas`)) / 전역 기본값(admin — 자동 적용 마스터 스위치·허용 시간대 cron·백분위/여유율/하한/임계/최소 절감/쿨다운/최대 감소폭/시스템 NS/opt-out annotation/Quota 임계) / 수집 스케줄(admin — 기본 cron + 클러스터 오버라이드).
  - 자동갱신 셀렉트(끔/15초~5분), `ExportMenu`(화면 캡처/내보내기 — 가상 스크롤 상태에선 뷰포트에 보이는 행만 캡처되고 CSV 는 전량).
  - **모든 카드는 프레임(MacCard)을 항상 렌더하고 내부만 로딩/실패/집계중/데이터로 교체**한다(early return 으로 통째 교체 금지 — 폴링마다 높이가 달라져 화면이 흔들린다). 집계 중에는 표 정렬을 동결(`useTableSort frozen`)해 부분 결과가 들어와도 행이 자리를 바꾸지 않는다.
- **Frontend(효율화)**: `hooks/useK8sEfficiency.ts` — `useEffRecommendations`/`useEffNsSeries`/`useEffRanking`/`useEffSummary`/`useEffQuotas`/`useEffPolicies`/`useEffPolicyDefaults`/`useEffSchedule`/`useEffRuns`/`useEffRun`(queued·running 동안 1.5s 폴링)/`useEffMutations`(collect·generate·apply·dismiss·rollback·quotaAdjust·customScale·savePolicy·deletePolicy·saveDefaults·saveSchedule). api.ts: `k8sEfficiencyApi`.
- **Backend(효율화)**: 라우터 `backend/app/routers/k8s_efficiency.py`(prefix `/k8s`) — 전역 `GET/PUT /k8s/efficiency/schedule`(PUT admin), `GET/PUT /k8s/efficiency/policy-defaults`(PUT admin), `GET /k8s/efficiency/runs/{run_id}`(폴링), `POST .../runs/{run_id}/rollback`(operator); 클러스터별 `POST /k8s/{id}/efficiency/collect`(operator → Celery), `GET .../history/namespaces?namespace&range`, `GET .../history/ranking?range&metric&top`, `GET .../history/summary`, `GET .../history/workloads`, `GET .../quotas`(라이브), `GET .../recommendations?status&namespace`, `POST .../recommendations/generate`, `POST .../recommendations/{id}/dismiss`, `POST .../apply`(`recommend_only` 포함 시 422), `POST .../quota/adjust`, `POST .../custom-targets/scale`, `GET/PUT/DELETE .../policies[/{namespace}]`, `GET .../runs`. 수집은 `services/k8s_efficiency/collector.py` 가 `k8s_allocation._build_overview` 를 재사용(같은 Pod 순회의 `on_pod` 훅으로 워크로드 집계, Redis 스냅샷 워밍). 추천 `engine.py`(p95: Prometheus `quantile_over_time` → DB `percentile_cont` → 데이터 부족), 적용/롤백 `apply.py`(strategic-merge patch, before 캡처, `dry_run="All"`), 쿼터 `quota.py`, 자동화 `automation.py`(전역 스위치·NS opt-in·쿨다운·최대 감소폭·허용 시간대). 설계 문서: `docs/02-design/k8s-efficiency-automation.md`.
- **Frontend**: `useAllocNodes`/`useAllocNamespaces`/`useAllocProgress`(페이지 루트 전용 — `select` 로 진행 메타만 구독)/`useAllocWorkloads`/`useAllocPods`/**`useAllocNodePods`**(노드 상세 lazy)/`useRefreshAllocNode`/`useRefreshAllocNamespace`/`useForceAllocRefresh`/`usePodsSummary` (`hooks/useK8sAllocation.ts`, 스냅샷 계열은 `HOLD_OPTS` — `keepPreviousData` + 자동 재페치 억제 + computing 시 1.5s 폴링, 드릴다운 계열은 `staleTime 30s` + 열렸을 때만 `enabled`). api.ts: `k8sAllocationApi.{nodes,node,nodePods,namespaces,namespace,workloads,pods,podsSummary}`. `services/api.ts` 응답 인터셉터가 **GET 의 502/503/504·네트워크 오류를 0.5s→1.5s 백오프로 2회 재시도**한다(멱등 요청만, `__noRetry` 로 opt-out).
- **Backend**: 라우터 `backend/app/routers/k8s_allocation.py`(prefix `/k8s`) — `GET /k8s/{id}/allocation/nodes`, `GET .../nodes/{node}`(노드 1건 metrics 단건 GET), `GET /k8s/{id}/allocation/namespaces`, `GET .../namespaces/{ns}`, `GET .../namespaces/{ns}/workloads`, `GET .../workloads/{kind}/{name}/pods`, **`GET .../nodes/{node}/pods`**(노드에 스케줄된 파드 전량 + 컨테이너 req/lim/usage + 상위 워크로드 귀속 — `spec.nodeName` field-selector 로 그 노드 파드만 받고, 노드에 걸린 NS 마다 metrics·ReplicaSet 을 **NS 단위로 병렬 조회**(8 workers, 캐시는 NS 드릴다운과 공유). NS 가 `K8S_ALLOC_NODE_DRILL_NS_MAX`(기본 64) 를 넘는 극단적인 노드만 cluster-wide metrics 1회 + 이름 기반 근사로 떨어지고 응답 `owner_approx` 로 화면에 안내), **`GET /k8s/{id}/allocation/pods-summary`**(개요 스냅샷에서 파생 — 요청 스레드가 apiserver 를 치지 않음). `refresh=true` 쿼리로 강제 재집계, 미완료 시 `status: "computing"` + `processed/total/progress` 로 부분 결과 스트리밍(폴링 기반). 스냅샷은 `services/snapshot_jobs.SnapshotManager` 가 **Redis 에 공유**(`K8S_ALLOC_SNAPSHOT_BACKEND=auto`, 클러스터당 빌더 1개 락, Redis 불가 시 프로세스 메모리 폴백)하므로 HPA 다중 replica 에서도 모든 파드가 같은 진행률/결과를 준다. 구 `GET /k8s/{id}/pods-summary`(`k8s_resources.py`)는 호환용(페이지 스트리밍으로 교체, deprecated).
- **핵심 기능**:
  ...
  - Pod 상태별(running/pending/error 등) 카운트 카드 — 같은 스냅샷 순회에서 계산(별도 전수 Pod 조회 없음).
  - 네임스페이스별 비효율(req−use) 랭킹 차트, 네임스페이스→워크로드→파드 드릴다운.
  - **기준 정보(노드/네임스페이스) 클릭 → 대상이 실제로 안고 있는 리소스(Deployment/StatefulSet/DaemonSet/Pod)의 자원 할당(request·limit) vs 실제 사용(usage) 상세** — 노드별 자원·네임스페이스별 자원·비효율 랭킹 세 탭이 같은 다이얼로그를 연다.
  - 개별 노드/네임스페이스 단위 즉시 재계산, CSV 내보내기, 화면 캡처 내보내기.
  - **자원 효율화**: 점유(request) 대비 사용률이 낮은 컨테이너의 request 축소 추천(p95 + 여유율) · 드라이런/적용/롤백 · NS 별 자동 적용 opt-in(쿨다운·최대 감소폭) · NS 자원 추이(request/실사용/Quota) 및 저효율 NS 추이 랭킹 · ResourceQuota 탄력(부족 시 확장, 지속 미사용 시 회수) · 오퍼레이터 CR 어댑터(StarRocks CN 등) · 실행 로그/감사.
- **요청사항 (수정 요청)**:
  - _(여기에 개선/수정 요청을 직접 적어주세요)_
  - **[2026-09-22] 노드/네임스페이스 클릭 → 실제 리소스 할당 vs 사용 상세** — 요청: 노드별 자원·네임스페이스별 자원·네임스페이스 비효율 랭킹 탭에서 기준 정보(노드/네임스페이스)를 클릭하면 대상의 실제 리소스(POD·STS 등)가 자원 할당(설정)과 실제 사용 상세로 보이게. 반영: 세 탭이 공유하는 `AllocDetailDialog` — 노드명(카드/테이블)·NS 명·랭킹 막대/Y축 라벨 클릭으로 열림. 노드 상세는 CPU/MEM 미터(allocatable 대비 req·use) + 워크로드(Deployment/STS/DS/Pod) → 파드 → 컨테이너별 req/limit/use·QoS·상태, NS 상세는 NS 스탯 + 같은 드릴다운. 노드 상세용 `GET .../nodes/{node}/pods` 신설. ([PR #709](https://github.com/riverjin839/devops_management/pull/709) · [목업](https://claude.ai/artifact/DXzm4mjy3jBXr17rHMpBqD))
  - **[2026-09-22] 요약 영역 압축 — 자원 탭을 화면 중앙으로, 스크롤 최소화** — 요청: 클러스터 요약 카드가 커서 노드별/네임스페이스별 자원 탭이 너무 밑에 나오고 잘려 보임. 카드 축소·공간 효율화, POD 용량/상태 카드는 없애거나 1/10 크기로. 반영: 기본 화면을 칩 한 줄 `SummaryStrip`(노드/NS/파드/배치여유/할당·사용효율/여유·낭비/Run·Pend·Err/req미설정)으로 대체 — POD 용량·상태는 칩으로 흡수. 기존 큰 카드(`SummarySection`+`PodCapacityStatusCards`, 스케줄 계산기 포함)는 `요약 상세` 토글(기본 접힘, localStorage 유지)로만 펼침. 탭 바 sticky. ([PR #709](https://github.com/riverjin839/devops_management/pull/709))
  - **[2026-09-23] 노드 상세 정확도 — NS 가 많은 운영 노드에서 사용량이 비던 문제** — 첫 반영에서는 노드에 걸린 NS 가 12개를 넘으면 cluster-wide metrics 1회로 떨어졌는데, 운영 워커는 DaemonSet 만으로도 NS 가 10개를 쉽게 넘고 대형 클러스터에서 cluster-wide metrics 는 타임아웃으로 빈 결과가 되기 쉬워 **노드 상세의 사용량 열이 통째로 비고** Deployment 귀속도 이름 추정으로 떨어졌다. NS 별 metrics·ReplicaSet 조회를 순차 → **병렬**(8 workers)로 바꾸고 상한을 64 로 올렸다. 상한을 넘는 극단적인 노드는 여전히 근사로 떨어지지만 이제 화면에 그 사실을 표시한다(`owner_approx`). 테스트 `tests/test_allocation_node_pods.py` 추가.
  - 잔여: 실 클러스터 확인 — ① metrics-server 없는 클러스터에서 "사용량 미가용" 폴백 ② NS 수십 개 노드에서 상세 응답 시간(병렬 8 workers 기준).
...
- **UI 구성**:
  ...
  - **터미널**(연결 후): `NodeSshTerminal` → 공용 `SshTerminalWindow`(xterm.js). 드래그 이동 플로팅 창(헤더가 드래그 핸들, CSS `resize` 로 크기 조절 — ResizeObserver 가 자동 리핏), 헤더에 재연결/새 창으로 빼기/전체화면/종료 + 복사/붙여넣기 단축키 안내. 색·글꼴은 Settings → 터미널 Appearance 의 활성 프로파일(`useXtermTheme`)을 따르고, **드래그 선택 후 Ctrl+C 복사 · Ctrl+V 붙여넣기**가 동작한다(선택이 없으면 Ctrl+C 는 SIGINT — `lib/terminalClipboard.ts`).
...
- **Backend**: `GET /api/v1/daily-check/results/{cluster_id}/latest`, `GET /api/v1/daily-check/results/{cluster_id}`, `POST /api/v1/daily-check/run/{cluster_id}` — `backend/app/routers/core_bundle_router.py`. `GET /api/v1/deep-check/review/{daily_check_log_id}`, `GET /api/v1/deep-check/trend/{cluster_id}`, `POST /api/v1/deep-check/run/{cluster_id}` — `backend/app/routers/check_results_router.py` (`DeepCheckService`, `ReviewService`). 모델: `DailyCheckLog`(`core_bundle_snapshot.py`), `DeepCheckResult`(`check_definitions.py`, `ai_summary`/`ai_diff`/`ai_trend` 필드는 `DailyCheckLog` 레벨).
- **핵심 기능**:
...
- **Backend**: `backend/app/routers/check_definitions_router.py` — `GET/POST /api/v1/deep-check/definitions`(`with_status` 시 정의별 최신 결과 요약 포함), `PUT/DELETE /definitions/{id}`(admin, 삭제 시 이력은 `definition_id=NULL` 로 보존), `POST /definitions/{id}/duplicate`(admin, 비활성 복제), `POST /definitions/{id}/test`(operator, 무기록), `POST /definitions/{id}/run`(operator, `DeepCheckResult` 영속), `GET /definitions/{id}/results`(이력 페이지네이션 + status 필터), `POST /definitions/preview`(operator, 저장 전 ad-hoc), `GET /check-types`. `check_type` 은 `app/services/registered_checks/REGISTRY` 화이트리스트 검증, `schedule_cron` 은 `validate_cron_min_interval`(5분 미만 거부). 모델: `DeepCheckDefinition`(+`last_run_at`), `DeepCheckResult`.
- **핵심 기능**:
...
- **핵심 기능**:
  ...
  - **SCP 업로드 내용 — 다른 노드에서 불러오기**: 로컬 파일 선택과 나란히, 현재 화면에 로드된 임의 노드 하나를 골라 원격 경로의 텍스트 파일 내용을 그대로 가져와 업로드 입력창을 채운다(가져온 뒤 수정 후 다른 노드들에 재배포 가능). 업로드 대상(targets) 인증 정보를 그대로 재사용, 별도 저장 없음. 옆의 **"로컬에 저장"** 버튼으로 현재 업로드 내용(어디서 채웠든)을 브라우저 다운로드로 내 컴퓨터에 파일로 저장 가능(파일명은 원격 경로의 마지막 세그먼트 재사용, BOM 없이 저장해 셔뱅 등 스크립트 실행에 영향 없음)
- **요청사항 (수정 요청)**:
  - _(여기에 개선/수정 요청을 직접 적어주세요)_
...
### Batch Jobs (홈 `/` → **플랫폼 현황** 탭의 **배치잡** 서브탭, 구 독립 라우트 `/batch-jobs`)

> **화면 병합(2026-08)**: 독립 사이드바 메뉴였던 Batch Jobs 를 삭제하고 홈의 "플랫폼 현황" 탭
> 안 서브탭으로 접었다 — 둘 다 클러스터 단위 운영 화면이라 화면 수를 줄인 것뿐, 데이터
> 모델/백엔드는 무변경이다(배치잡은 상태를 바꾸는 "액션", 점검 매트릭스는 읽기 전용
> "상태 조회"라 매트릭스 셀 자체로 흡수하지는 않았다 — `HomePage.tsx` 안에서 컴포넌트를
> 그대로 재사용). 진입: 홈 → `플랫폼 현황` 탭 → `배치잡` 서브탭(Settings 접근 제어에서
> `/batch-jobs` 를 끄면 이 서브탭 버튼 자체가 안 보임 — `useNavCatalog().featureAllowed`).
> 사이드바 `DevOps` 그룹 메뉴에서는 빠졌다. 구 URL `/batch-jobs` 는 `/` 로 리다이렉트되고,
> `panelRegistry.ts` 의 Island 패널 등록(`BatchJobsPage` 직접 임베드)은 라우트와 무관해
> 그대로 유지된다. `BatchJobsPage` 는 원래 독립 라우트 페이지라 전체화면 셸을 두르는데,
> `HomePage.tsx` 가 `.island-embed` 래퍼(Your Island 임베드와 동일 패턴, `index.css`
> `.island-embed :is(.min-h-screen,...)` 규칙)로 그 셸을 무력화해 서브탭 안에 자연스럽게
> 앉힌다.

- **파일**: `frontend/src/pages/BatchJobsPage.tsx`(현재 `HomePage.tsx` 가 임베드) (+ `components/batch-jobs/BatchJobTable.tsx`, `BatchJobRow.tsx`, `BatchJobClusterGroup.tsx`, `CronBadge.tsx`, `BatchJobFilters.tsx`, `BatchJobSlideOver.tsx`(+`.EditForm`/`.RunForm`/`.RunHistory`/`.SavedCreds`), `BatchJobLogDetail.tsx`, `CreateBatchJobWizard.tsx`(+`.StepType`/`.StepHost`/`.StepSchedule`), `UnregisteredTypeChips.tsx`, `StatusPill.tsx`, 공통 `ClusterSidebar`/`ConfirmDialog`/`ExecOutputTabs`/`useToast`/`ui/tooltip`)
...
- **UI 구성**:
  ...
  - `MacCard "배치 잡"`: `BatchJobFilters`(상태 필터 + 검색), 체크박스 다중 선택 시 일괄 실행 바(저장된 자격증명으로 `bulkRun`). **전체 모드**에서는 잡이 등록된 클러스터별로 `BatchJobClusterGroup` collapsible 섹션(클러스터명·region·운영등급·잡/실패/실행중 통계 헤더 + 섹션별 `BatchJobTable` + `UnregisteredTypeChips`)으로 그룹핑되고, **단일 클러스터 선택**시에는 기존처럼 flat `BatchJobTable` + `UnregisteredTypeChips`. 그룹 섹션 테두리+상태 dot 는 cron 건강 상태(`filters.ts` `aggregateCronHealth`)로 착색 — 정상 초록/비정상 레드/중지 회색/실행 중 블루, 접힌 상태에서도 판독 가능.
  - `BatchJobTable` 각 행(`BatchJobRow`): 좌측 보더가 cron 건강 상태(`cronHealth`) 색으로 tint(선택 시 primary), cron 컬럼은 `CronBadge`(색상 코드 배지 — 등록됨/대기 중/평가 오류/자격증명 없음/꺼짐 + hover 툴팁으로 cron 식·활성화·저장 자격증명·스케줄러 최근 평가·최근 실행 노출), 우측 "실행" 컬럼에 상태에 따라 ▶(즉시 실행, hover 로 잡 상세 툴팁) 또는 ■(중지, 실행 중일 때만) 아이콘.
  - 우측 `BatchJobSlideOver` — 행 클릭 시 잡 상세(수정/실행/이력/자격증명), 뷰포트 <1280px에서는 overlay 모드. 실행 이력 위에 **"최근 실행 로그"**(`BatchJobLogDetail` — mc 클라이언트 콘솔과 동일한 상태/트리거/실행자/호스트/exit/시간 sticky 헤더 + **실행 단계 타임라인**(`ExecutionStepsTimeline` 재사용, 타입 descriptor 의 `stepPlan`+run 의 `steps`) + **실측 명령 trace**(`CommandTraceList` — kind 배지/exit/duration/펼침 출력) + `ExecOutputTabs`)가 펼치지 않아도 항상 노출. 실행 중인 잡은 액션 바의 "지금 실행" 버튼이 "지금 중지"로 바뀐다.
  - `CreateBatchJobWizard`(3단계: 타입 → 호스트 → 스케줄) — 1단계에 **실행 방식** 토글(시스템 제공/스크립트 선택, Phase 2): "스크립트 선택" 시 `/scripts` 라이브러리의 shell/ansible_playbook kind 스크립트만 고를 수 있고(python 은 Phase 2 K8s Job 실행기 대기 — 목록에서 제외), 버전 고정(항상 최신/특정 버전) 선택 가능. 등록 후에는 job_type(및 스크립트 연결)이 불변이라 EditForm/슬라이드오버 헤더는 잡 타입 대신 스크립트 이름을 표시. 삭제/중지 시 `ConfirmDialog`(위험 확인).
- **Frontend**: `useBatchJobTypes`, `useBatchJobs`, `useDeleteBatchJob`(+ 컴포넌트 내부에서 `useCreateBatchJob`/`useUpdateBatchJob`/`useRunBatchJob`/`useStopBatchJob`/`useTestBatchJobConnection`/`useBatchJobRuns`(15초 폴링), 모두 `hooks/useBatchJobs.ts`), `useClusters`. 순수 로컬 state(Zustand 스토어 없음)로 `selectedClusterId`, `statusFilter`, `search`, `sort`, `selectedJob`, `wizardCtx`, `selectedIds`(일괄 실행용), `collapsedClusterIds`(그룹 접기), `overlayMode`(matchMedia) 관리. 일괄 실행은 훅이 아닌 `batchJobsApi.bulkRun()` 직접 호출 + `useToast`로 결과 알림. cron 배지/즉시 실행·중지 아이콘의 hover 는 shadcn `Tooltip`(Base UI, `components/ui/tooltip.tsx`) — `BatchJobTable` 루트를 `TooltipProvider` 로 감싼다.
- **Backend**: `GET /api/v1/batch-jobs/types`(등록된 job_type 목록, `requires_ssh` 포함), `GET/POST /api/v1/batch-jobs`, `PUT/DELETE /api/v1/batch-jobs/{id}`, `POST /api/v1/batch-jobs/{id}/run`(동기, timeout 600s), `POST /api/v1/batch-jobs/{id}/stop`(실행 중지 — 아래 참고), `POST /api/v1/batch-jobs/bulk-run`(Celery `run_batch_job.delay()`로 비동기 큐잉, 저장된 자격증명 없으면 스킵), `GET /api/v1/batch-jobs/{id}/runs`, `GET /api/v1/batch-jobs/runs/{id}`, `POST /api/v1/batch-jobs/{id}/test-connection`(SSH 잡은 SSH 연결 검증, non-SSH 잡은 K8s 사전 점검 `_k8s_preflight` — kubeconfig 해석→kubectl 바이너리→인증 /healthz→RBAC can-i 를 `mode:"k8s"`+`checks[]` 로 반환) — 전부 `backend/app/routers/batch_jobs.py`. create/update/delete/run/bulk-run/stop 은 `audit_logger.record()`(action=`batch_job.*`)로 감사 로그도 남긴다(admin 이 Settings 감사 로그 조회에서 누가/언제/무엇을 했는지 추적). 서비스: `app/services/batch_job_service.py`(`execute_job`, `get_job_or_404` — 수동/일괄 실행 시 요청 사용자를 `triggered_by_user_id`/`triggered_by_username` 로 전달; `_run_and_record` 가 실행 *전*에 BatchJobRun 을 status="running" 으로 먼저 만들어 중지 대상이 항상 존재하게 함), `app/services/batch_jobs/`(`@register_executor` 패턴으로 job_type별 executor 등록, `list_executors`/`get_executor`; SSH executor `etcdctl_defrag`/`shell_command` + non-SSH(`requires_ssh=False`) executor `k8s_job_cleanup` — 클러스터 kubeconfig 로 kubectl 직접 실행, dry_run 기본; `script_executor.py`(`job_type="script"`, Phase 2) — `BatchJob.execution_mode="script"` 인 잡 전용, 실제 shell/ansible 내용은 `execute_job()` 이 `script_id`/`script_version_id`(null=항상 최신)로 `ExecutableScript(Version)` 을 로드해 `ExecutionContext.script_kind/script_content/script_inventory_content` 에 주입하고 `BatchJobRun.script_version_id` 로 실제 실행된 버전을 스냅샷; `base.py` 의 `CancelToken` 이 SSH client/kubectl Popen 을 attach 해 중지 요청 시 강제 종료), `app/services/active_runs.py`(수동 실행용 in-process 취소 레지스트리), `app/services/ssh_runner.py`(paramiko 기반 `test_connection`, `_exec_ssh`/`run_bulk` 가 `cancel_token` 을 받아 SSH client 를 attach), `app/services/secret_box.py`(자격증명 암/복호화). 모델: `backend/app/models/batch_job.py`(`BatchJob` — cron·`encrypted_password`/`encrypted_private_key`·`last_schedule_check_at`/`last_schedule_note`·`active_task_id`(Celery revoke 대상 추적)·**`execution_mode`(system/script)·`script_id`/`script_version_id`(Phase 2 — FK, null 버전=항상 최신)**; `BatchJobRun` — `triggered_by_user_id`/`triggered_by_username`(실행자 스냅샷)·`params_snapshot`(그 실행에 실제로 쓰인 merge 후 파라미터, admin 감사용)·**`steps`/`commands`(JSONB — 단계별 실행 trace + 실측 명령 기록, deep check `ExecutionStep` 과 동일 shape, 예외 경로 포함 영속)**·**`script_version_id`(스크립트 기반 잡이 이 실행에서 실제로 사용한 버전 스냅샷)**, status 에 `cancelled` 추가). 실행기 계측: `services/batch_jobs/base.py` 의 `_step()`/`_record_command()`/`step_plan`(타입별 정적 단계 계획, descriptor 로 노출), kubectl 실패는 `services/k8s_diagnose.py` `classify_kubectl_failure` 로 connect_error/auth_error/error 분류, kubeconfig 실패 사유는 `services/kubeconfig.py` `resolve_kubeconfig` 가 생성.
- **핵심 기능**:
  - 클러스터 단일 선택/전체 보기(전체는 클러스터별 그룹 섹션), 상태 필터(전체/실패/실행중 등) + 텍스트 검색
  - 체크박스 다중 선택 후 저장된 자격증명으로 여러 클러스터 잡을 Celery 백그라운드 일괄 실행(`bulk-run`), 큐잉/스킵 결과를 토스트로 안내
  - 잡별 슬라이드오버에서 수정/수동 실행(요청 시 자격증명 override 가능)/최근 실행 로그(`BatchJobLogDetail`)/실행 이력(`BatchJobSlideOver.RunHistory`)/저장된 자격증명 관리(`.SavedCreds`)
  ...
  - 연결 테스트(`test-connection`): SSH 잡은 SSH 자격증명/네트워크 검증, non-SSH 잡은 "사전 점검" — kubeconfig/kubectl/인증 healthz/RBAC 단계별 결과를 실행 폼에 행으로 표시
  - **단계별 실행 추적**: 실행 결과·이력 카드에 실행 단계 타임라인 + 실측 명령 trace 표시 — 어느 단계에서 실패했는지, 실제 어떤 명령이 나갔는지(exit/출력 발췌) 판독. kubectl 실패는 연결/인증/기타로 분류돼 상태 pill 에 반영
  - 실행 이력 15초 자동 갱신 + 트리거 배지(수동/스케줄/일괄), 일괄 실행은 trigger="bulk" 로 기록
  - **admin 실행 추적성**: 실행자(수동/일괄만, 스케줄은 시스템)와 그 실행에 실제로 쓰인 파라미터 스냅샷(dry_run 여부 등)이 로그 상세 카드에 노출되고, 등록/수정/삭제/실행/중지는 감사 로그에도 기록
  - **실행 중지**: 테이블 행 또는 슬라이드오버에서 실행 중인 잡을 즉시 중지 — 수동 실행은 SSH 채널/kubectl 프로세스를 직접 닫아, 스케줄/일괄은 Celery revoke(terminate)로 중단. 실제 강제종료 성공 여부와 무관하게 DB 상태는 항상 `cancelled` 로 정정됨
  - **행별 즉시 실행**: 잡을 열지 않고 테이블에서 바로 실행하는 ▶ 아이콘(저장 자격증명/non-SSH 필요, 없으면 비활성화+안내). **`enabled=false`(비활성화)면 스케줄뿐 아니라 이 ▶ 버튼과 슬라이드오버 "실행" 폼도 함께 막힌다**(서버 `POST /{id}/run` 도 400 으로 동일하게 거부 — 이전엔 스케줄 디스패처/일괄 실행만 `enabled` 를 지켰고 이 즉시 실행 경로만 검사가 빠져 있었다). 꺼진 잡은 이름 아래 `PowerOff` 아이콘 + "사용 안 함" 배지로 표시되고, ▶ 버튼/실행 폼 모두 비활성화 사유(꺼짐 vs 자격증명 없음)를 구분해 툴팁으로 안내한다. "꺼짐"의 의미는 **앞으로의 새 실행(스케줄+수동)만 막는 것**이고 이미 클러스터에 떠 있는 K8s 리소스는 건드리지 않는다(`k8s_job_cleanup` 은 애초에 영속 Job/CronJob 리소스를 만들지 않고 `kubectl get/delete jobs` 만 수행하므로 "꺼도 남아있는 리소스"가 존재하지 않는다).
...
- **파일**: `frontend/src/pages/WorkItemBoardPage.tsx` (+ `components/work-items/WorkItemKanban.tsx`, `WorkItemCalendar.tsx`, `WorkItemTableRow.tsx`, `WorkItemEpicView.tsx`, `WorkItemActionsMenu.tsx`, `ColumnSettingsMenu.tsx`, `WorkItemBoardSettingsModal.tsx`, `JiraImportModal.tsx`, `JiraProvisionModal.tsx`, `JiraLinkDialog.tsx`, `ServiceNowRegisterDialog.tsx`, `JiraIssueChip.tsx`, `DocLinkChip.tsx`, `ConfluenceLinkCell.tsx`, `components/dashboard/QuickAddTaskModal.tsx`)
- **목적 / UX**: 전사 업무(유형: 이슈 대응/회의/운영 대응/기타 — 내부 저장값은 `issue`/`meeting`/`task`/`etc`, 레거시 `training` 도 조회는 계속 됨)를 표(목록)/달력/칸반/에픽뷰 4가지 뷰로 조회·필터링·정렬하고, 등록·수정·삭제·CSV 추출·Jira 가져오기까지 처리하는 업무 관리의 메인 허브.
- **UI 구성**:
  - 헤더: 제목 옆 **Settings 아이콘**(`WorkItemBoardSettingsModal`, admin 전용 — 일반 사용자는 아이콘 자체가 안 보임)을 클릭하면 (1) 보여주기 뷰(목록/달력/칸반/에픽뷰) 노출 여부 + 기본 뷰, (2) 헤더 배지(전체/WIP/Done/지연) 노출 여부를 전 사용자 공통으로 설정할 수 있다 — 기본값은 에픽뷰+목록만 노출(기본 뷰=에픽뷰, 달력·칸반은 숨김), 배지 4개는 전부 숨김. 전체/WIP/Done/지연 카운트 배지는 이 설정에 따라 조건부 노출되고, 뷰 전환(`ViewModeBar`)도 노출 설정된 뷰만 탭으로 뜬다. "JIRA" 버튼(Jira 가져오기, `jiraConfig.enabled`일 때만) · "Confluence" 버튼(Confluence 연동, `jiraConfig.confluenceBaseUrl` 설정 시만) · "CSV" 버튼(CSV 추출) — 셋 다 버튼 라벨은 짧게 줄이고 전체 설명은 `title` 툴팁으로 옮겼다 · 업무 등록 버튼
  - 필터 바 — **업무 분류 드롭다운**(4개 유형) · **상태 드롭다운**(칸반 상태) · 제목 검색 · **내 업무 토글** · 담당자·우선순위·**등록 타입**·모듈·스프린트·기간(**이번주→2주→이번달→해제 4단 순환** 버튼, from~to)까지 **전부 한 줄에 인라인으로** 노출한다("필터 더보기" 팝오버로 묶지 않음). 우측(`ml-auto`)엔 컬럼 설정 메뉴만 남음(시간표시 토글·사용자 정의 필드 관리 버튼은 제거됨). **기본 필터링은 항상 아무 조건도 없는 상태로 시작** — "필터 저장" 버튼을 눌러야 지금 조합이 사용자별로 저장돼 다음 방문에 복원된다(값이 바뀔 때마다 자동 저장되던 이전 동작 폐지). **등록 타입 필터**(`jiraIssueType`, `GET /work-items` `jira_issue_type` 쿼리파라미터 — ILIKE 정확 일치)는 Jira 이슈 종류가 프로젝트마다 달라 고정 enum 이 아니라, 지금까지 어떤 조회로든 걸려든 값을 계속 누적해(`seenJiraTypes`) `<select>` 옵션으로 쓴다(값이 하나도 없으면 셀렉트 자체를 숨김) — 등록 타입으로 좁혀 걸었을 때 그 값 하나만 남아 다른 선택지가 사라지는 걸 막기 위함
  - 목록 뷰: dnd-kit 기반 컬럼 드래그 정렬 + 행 드래그 정렬(로컬 순서, `useLocalOrder`), 컬럼 리사이즈(`useColumnWidths`)·표시여부(`useColumnLayout`) — **컬럼 폭/순서/표시여부도 필터와 동일하게 계정별 localStorage 키(`item-board-table-v4:{username}`)로 분리 저장**. 기본 컬럼 순서는 상태·담당자·등록 타입·DL#·WIKI·작업제목·시작일·마감일·변경·업무 분류(그 외는 기본 숨김 — **상위업무 포함**, 컬럼 설정에서 켬). **컬럼 폭은 하드코딩 기본값이 아니라 실제 셀 내용(`data-col` 마커의 `scrollWidth`) 기준 자동맞춤** — 데이터가 처음 로드된 직후, 아직 아무도 손대지 않은(드래그/자동맞춤 이력 없는) 컬럼만 내용 기준 폭으로 맞춰 그대로 고정·저장한다(`useColumnWidths.autoFitMissing`). 리사이즈 그립을 더블클릭하면 그 컬럼만 지금 렌더된 내용 기준으로 다시 재고 고정한다(`autoFit`, 예전엔 하드코딩 기본값으로 되돌리기만 했음). **컬럼 설정 박스**(`ColumnSettingsMenu`)에 "현재 설정을 기본값으로 저장"(디바운스 우회 즉시 저장 + 토스트 확인) 버튼이 있고, "기본값으로 복원"은 순서·표시여부·폭을 모두 함께 초기화한다(예전엔 폭이 복원 대상에서 빠져 있었음). 등록은 헤더의 "업무 등록" 버튼(`QuickAddTaskModal`) 하나로 통일
  - **"변경" 컬럼**(구 "관리"): 전체 수정·Jira/Confluence 생성·재가져오기·보내기·연결 관리·동기화·하위 업무 추가·삭제까지 최대 8개였던 개별 아이콘을 대표 아이콘(`Settings2`) 하나로 접었다 — hover(또는 클릭)하면 아이콘·색상·라벨을 갖춘 드롭다운 목록이 열린다(항목별 조건부 노출은 기존과 동일: Jira 미연결이면 자동 생성만, 연결됐으면 재가져오기/보내기/연결 관리, `provisionStatus==='partial'`이면 트리거에 경고 점 + 재시도 항목이 노란색). 트리거+드롭다운 로직은 `WorkItemActionsMenu`(목록 뷰·에픽뷰 공유)로 분리돼 있다. 표가 가로 스크롤 + 모서리 라운딩용 `overflow-hidden` 컨테이너 안에 있어 드롭다운을 inline `absolute` 로 열면 잘리므로, `SearchableSelect`의 `menuPortal` 과 동일하게 `document.body` 로 portal + `getBoundingClientRect()` 기반 `position: fixed` 앵커링을 쓴다. 바깥 클릭 닫기는 전체화면 오버레이 대신 `mousedown` 전역 리스너로 처리한다 — 오버레이 방식은 트리거 버튼 위에 얹혀 hover 이벤트를 가로채면서 열림→닫힘이 반복되는 깜빡임 버그가 있었다
  ...
  - **WIKI 컬럼**: Confluence 문서 링크 — `ConfluenceLinkCell`(목록 뷰·에픽뷰 공유) 이 `DocLinkChip`(대표 링크가 없으면 점선 `＋문서` 버튼, 클릭하면 그 자리에서 URL 입력·저장)을 감싸고, Jira 원격 링크에서 찾은 Confluence 페이지가 2개 이상이면 "N" 배지 + 클릭 시 전체 목록 드롭다운(제목·새창 열기)도 함께 보여준다. DL#/WIKI/등록 타입 세 컬럼은 칩 하나만 들어가는 좁은 컬럼이라 오른쪽 패딩을 줄이고(`tightRight` 메타) 폭 자체도 내용 기준 자동맞춤(위 목록 뷰 UI 구성 참고)이라 오른쪽에 여백이 남지 않는다
  - **마감일** 컬럼(기본 노출): Jira 연동 업무는 `duedate` 동기화, 미연동 업무는 직접 편집(인라인 date input). 마감 지났고 미완료(`kanbanStatus !== 'done'`)면 빨간색 강조
  - **담당자 컬럼**(구 "담당자(정/부)"): 정/부 두 명을 함께 편집하던 구조에서 **실제 담당자 한 명만 지정하는 단일 편집 셀**로 단순화됐다(`primaryAssignee`/`assignee` 동시 갱신, 클릭 → 인라인 텍스트 입력). `secondaryAssignee` 필드 자체는 남아 있고 칸반/에픽뷰/멤버보드 등 다른 화면은 여전히 참조한다 — 이 컬럼에서만 편집 UI 가 빠졌다
  - Jira 원본 축 컬럼: **상위업무**(구 "Epic", **기본 숨김** — Epic→Task 체인이 필요하면 에픽뷰를 쓰거나 컬럼 설정에서 켠다) · 등록 타입(구 "이슈 종류", 기본 노출) · 컴포넌트 · 라벨(둘 다 기본 숨김). **"상위업무" 는 Epic→Task 체인을 함께 보여준다** — Epic 과 상위(Task) 가 둘 다 있고 서로 다르면 `JiraIssueChip` 2개를 화살표로 이어 표시(Epic → Task), 하나만 있으면 그 칩만(`jiraEpicKey`/`jiraParentKey` 동시 참조). **"상태" 컬럼이 Jira 상태를 겸한다** — Jira 연결 업무면 칸반 라벨 대신 **Jira 원본 상태명**을 보여주고 점 색은 `statusCategory` 기준(별도 "Jira 상태" 컬럼 없음 — 중복이라 병합)
  - 칸반 뷰: `WorkItemKanban` (상태별 컬럼)
  - 달력 뷰: `WorkItemCalendar`
  - **에픽뷰**: `WorkItemEpicView` — Epic → Task → Sub-task 순으로 정렬한 표. Epic 은 **진짜 Epic Link 값**(`jiraEpicKey`, 없으면 레거시 합본 문자열 `jiraEpic`)으로만 묶는다 — Sub-task 의 상위(parent) Task 를 Epic 대용으로 쓰지 않는다(Jira Server/DC 는 team-managed 프로젝트가 없어 `parent` 는 항상 "상위 Task" 관계일 뿐 Epic 이 아니다; 예전엔 Epic Link 값이 없으면 상위 Task 를 그대로 Epic 으로 취급해 실제로는 Epic 이 아닌 일반 Task 가 그룹 헤더로 잘못 노출되는 버그가 있었다 — `extract_epic_parts`/`jira_service.py`). Task/Sub-task 는 `parentId`(PEP 자체 상하위) 또는 `jiraParentKey`→`jiraIssueKey`(Jira Task→Sub-task) 체인으로 재귀 구성한다 — 상위가 현재 필터 결과에 없으면(다른 조건으로 빠졌거나 안 가져온 경우) 그 항목을 최상위로 취급한다("상위업무" 컬럼의 fallback 표시와 동일 원칙). Epic 없는 업무는 실제 Jira 구조 그대로 맨 뒤 "에픽 없음" 그룹으로 모아 목록에서 빠지지 않는다. Epic 그룹·각 노드 모두 기본 펼침이고 개별 접기/펼치기가 가능(로컬 state, 새로고침 시 초기화 — 개인화 저장 없음). 목록 뷰와 달리 컬럼 커스터마이즈·드래그 정렬은 없는 고정 컬럼(작업/DL/WIKI/상태/담당자/우선순위/마감일/변경) — 구조 자체가 정렬 기준이라 칸반/달력처럼 별도의 읽기 중심 뷰로 둔다. WIKI 셀은 목록 뷰와 동일한 `ConfluenceLinkCell` 을 재사용한다(예전엔 이 뷰에 WIKI 컬럼 자체가 없어 목록 뷰에서 WIKI 를 켜도 에픽뷰엔 반영되지 않았다). "변경" 열은 `WorkItemActionsMenu`(대표 아이콘 + hover 드롭다운)를 목록 뷰와 공유 재사용한다.
...
- **Backend**: `GET /api/v1/work-items`(목록, 필터 쿼리파라미터 snake_case 변환 — `jira_issue_type`(등록 타입, ILIKE 정확 일치)도 포함), `POST /api/v1/work-items`, `DELETE /api/v1/work-items/{id}`, `GET /api/v1/work-items/export/csv` — `backend/app/routers/work_items.py`. 게시판 공통 설정은 `GET/PUT /api/v1/ui-settings/work-item-board`(`ui_settings.py`, `AppSetting` key `work_item_board_settings` — GET 은 인증만, PUT 은 `require_admin`). 값은 `view_visibility`(뷰별 bool 4개) · `default_view` · `badge_visibility`(배지별 bool 4개)이고, 전부 숨김이면 기본값으로 폴백, `default_view` 가 숨겨진 뷰를 가리키면 우선순위(에픽뷰→목록→칸반→달력)로 폴백한다(`_normalize_board_settings`). 프로젝트명/스프린트명 매핑을 위해 `GET /api/v1/projects`(`projects.py`), `GET /api/v1/sprints`(`sprint.py`)도 호출. Jira 가져오기 가능 여부는 `GET /api/v1/jira/config`(`jira.py`). 행 단위 동기화는 `POST /api/v1/jira/refresh/{id}` · `POST /api/v1/jira/push/{id}`, 연계 생성은 `GET /api/v1/jira/provision/defaults` · `POST /api/v1/jira/provision`. Confluence 상위 페이지 탐색은 `GET /api/v1/jira/confluence/page-info`(제목 조회) · `GET /api/v1/jira/confluence/children`(하위 페이지 목록). 연결 복구는 `POST /api/v1/jira/unlink/{id}` · `POST /api/v1/jira/relink/{id}` · `POST /api/v1/jira/verify-links`. 모델: `backend/app/models/work_item.py` (WorkItem), `backend/app/models/user_jira_credential.py`(`jira_username` — 담당자 매핑용 원본 Jira 로그인 계정). **Jira 가져오기 제목**: `map_jira_issue`(`services/jira_service.py`)와 엑셀 저장(`import_excel_save`) 모두 `title` 을 요약문만 저장한다(구 `"{키} {요약}"` 접두어 형식 제거 — 이슈 키는 화면에서 DL# 컬럼이 별도로 보여줌). Jira 로 되돌려 보낼 때 접두어를 떼는 `strip_issue_key_prefix` 는 과거 데이터 호환용으로 유지.
- **핵심 기능**:
  - 표/달력/칸반/에픽뷰 4뷰 전환 및 유형·상태·제목·담당자·우선순위·등록 타입·모듈·스프린트·시작일 범위 복합 필터(전부 한 줄 인라인 노출)
  ...
  - **Jira 동기화 확장** — 가져오기/재가져오기/재연결 시마다 `backend/app/routers/jira.py` 가 최신값으로 반영:
    ...
    - **스프린트**: Jira Sprint 커스텀필드(설정 `jira_sprint_field`, Epic Link 필드와 같은 방식) 이름을 PEP `Sprint.name` 과 대소문자 무시로 매칭 — 매칭 실패 시 자동 생성 없이 스킵
    - **상위업무 체인**: Sub-task 자신에게 Epic Link 값이 없으면 상위(Task) 를 1회 추가 조회해 그 Epic 값을 가져온다(형제 Sub-task 가 같은 상위를 공유하면 배치당 1회만 조회)
  ...
  - 행 단위 Jira 재가져오기 / Jira 로 보내기 / Jira·Confluence 연계 생성(Epic·Sub-task 상위 이슈 지정, 기준 조건은 사용자별로 기억). **담당자**: 기본은 로그인 사용자 자신(본인 Jira 계정이 연동돼 있을 때만 채워짐)이고, 화면에서 본인 Jira 계정을 연동한 다른 PEP 사용자로 바꿀 수 있다(`UserJiraCredential.jira_username` — 표시명이 아닌 원본 로그인 계정으로 매핑, 검증되지 않은 이름 매칭은 하지 않는다). **Confluence 상위 페이지 ID**: mouseover 하면 그 페이지 제목을 툴팁으로 보여주고(디바운스 조회), "가져오기" 버튼으로 하위 페이지 목록을 조회해 그중 하나를 새 상위로 선택할 수 있다(선택한 페이지 밑에 문서 생성)
  ...
  - **ServiceNow ITSM 등록**(`ServiceNowRegisterDialog`, "변경" 메뉴 또는 상세 페이지 버튼, `jiraIssueKey` 있는 업무에서만 노출): Jira 연동 정보를 그대로 사용해 사내 ServiceNow 에 티켓을 수동 생성하고 결과(티켓 번호/링크)를 업무에 연결한다. 등록은 짧은 다단계 서버 호출이라 SSH 콘솔이 아니라, 응답에 담긴 단계별 로그(`steps`)를 "로그 보기" 토글로 펼쳐 `LogViewer` 에 보여준다(CLAUDE.md 의 "모든 실행 버튼은 상세 로그 + 로그 보기 옵션" 규칙 준수 — `NodeSpecPage.tsx` Host Facts 수집 버튼과 동일 패턴). 인증 세션이 없으면(`authIssue`) Settings → 연동(Jira) 안내 배너를 띄운다.
  ...
  - **Settings(admin 전용)**: 보여주기 뷰(목록/달력/칸반/에픽뷰)와 기본 뷰, 헤더 배지(전체/WIP/Done/지연) 노출 여부를 전 사용자 공통으로 설정. 최소 1개 뷰는 항상 켜져 있어야 하고, 기본 뷰가 꺼지면 자동으로 다른 켜진 뷰로 넘어간다
- **요청사항 (수정 요청)**:
  - _(여기에 개선/수정 요청을 직접 적어주세요)_
...
- **UI 구성**:
  ...
  - 가져오기 위저드: 간편(스페이스/상위 페이지 ID/기여자/라벨/기간/문서 제목/검색어)/CQL 검색 → dry-run 미리보기(create/update/unchanged 배지 + changes/경고 펼침, 행 선택) → 커밋 결과(신규/갱신/건너뜀/경고). 기여자 기본값은 본인(수정 가능), 기간 기본값은 이번주(월요일부터, Jira 가져오기와 동일 패턴)
