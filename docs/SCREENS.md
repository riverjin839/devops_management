...
네트워크·스토리지·DevOps)은 좌측 사이드바 레일, `work`(협업·문서 관리)은 상단 `AppTopBar`,
`system`(Settings)은 레일 하단 개인 존. 예전엔 홈 모드(work/platform)가 이 배치 자체를
게이팅해 반대 도메인 그룹이 사이드바에서 통째로 사라졌지만(R-4 5차 D-054), 지금은 `domain` 이
