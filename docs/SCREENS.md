...
- **Backend**: 라우터 `backend/app/routers/k8s_allocation.py`(prefix `/k8s`) — `GET /k8s/{id}/allocation/nodes`, `GET .../nodes/{node}`(노드 1건 metrics 단건 GET), `GET /k8s/{id}/allocation/namespaces`, `GET .../namespaces/{ns}`, `GET .../namespaces/{ns}/workloads`, `GET .../workloads/{kind}/{name}/pods`, **`GET .../nodes/{node}/pods`**(노드에 스케줄된 파드 전량 + 컨테이너 req/lim/usage + 상위 워크로드 귀속 — `spec.nodeName` field-selector, NS 수가 `K8S_ALLOC_NODE_DRILL_NS_MAX` 이하면 NS 단위 metrics/ReplicaSet 조회로 정확히, 넘으면 cluster-wide metrics 1회 + 이름 기반 근사), **`GET /k8s/{id}/allocation/pods-summary`**(개요 스냅샷에서 파생 — 요청 스레드가 apiserver 를 치지 않음). `refresh=true` 쿼리로 강제 재집계, 미완료 시 `status: "computing"` + `processed/total/progress` 로 부분 결과 스트리밍(폴링 기반). 스냅샷은 `services/snapshot_jobs.SnapshotManager` 가 **Redis 에 공유**(`K8S_ALLOC_SNAPSHOT_BACKEND=auto`, 클러스터당 빌더 1개 락, Redis 불가 시 프로세스 메모리 폴백)하므로 HPA 다중 replica 에서도 모든 파드가 같은 진행률/결과를 준다. 구 `GET /k8s/{id}/pods-summary`(`k8s_resources.py`)는 호환용(페이지 스트리밍으로 교체, deprecated).
- **핵심 기능**:
