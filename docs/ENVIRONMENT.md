...
| `K8S_ALLOC_NODE_DRILL_NS_MAX` | `12` | 동일 — 노드 상세(`GET .../allocation/nodes/{node}/pods`)에서 이 수 이하의 네임스페이스가 걸려 있으면 NS 단위 metrics·ReplicaSet 조회(정확 + NS 드릴다운과 캐시 공유), 넘으면 cluster-wide metrics 1회 + 이름 기반 워크로드 근사로 전환. NS 가 흩어진 노드에서 호출 수가 선형으로 늘지 않게 하는 스위치 |
