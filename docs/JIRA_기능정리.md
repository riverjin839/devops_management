...
| **SSO 자동 로그인** — 파드 내 폼 로그인 | 브라우저(Chromium/Playwright) 없이 httpx 로 SSO 리다이렉트/폼 체인을 수행해 세션 쿠키를 캡처·저장. Confluence URL 설정 시 **한 번의 IdP 로그인으로 Jira+Confluence 세션 동시 캡처**. 세션 만료(401) 시 저장된 로그인 정보(옵트인)로 자동 재로그인 | `POST /jira/sso/login`, `services/jira_sso_http.py` `sso_login_products()` |
