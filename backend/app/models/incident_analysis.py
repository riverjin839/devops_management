"""IncidentAnalysis — 알람/수동 트리거로 실행된 AI 장애 분석 결과.

알람(`alert_events`) 수신 시 분석 범위 규칙(AppSetting ``llm_analysis_scope``)에
매칭되면 전용 Celery ``llm`` 큐에서 비동기로 분석해 이 테이블에 남긴다.
...
class IncidentAnalysis(Base):
    ...
    # FK 를 걸지 않는다 — 알람이 retention 으로 먼저 지워져도 분석 이력은 조회 가능해야 한다.
    ...
    trigger = Column(String(16), nullable=False, default="alert")   # alert | manual
    ...
    def __repr__(self) -> str:
        return f"<IncidentAnalysis(alert={self.alert_event_id}, status={self.status})>"
