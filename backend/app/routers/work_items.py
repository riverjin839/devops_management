...
#
# ⚠️ 신원 식별자 불일치:
#   로그인 username 은 담당자 자동계정의 경우 **사번(employeeId)** 이다. 수동 생성 계정은
#   임의 username. 그런데 work item 의 담당자 필드(primary/secondary_assignee)에는 담당자
#   **이름**이 저장된다(WorkItemForm 이 assignee.name 을 값으로 사용). 신원 키가 필드마다 다르다.
#
# 정책(타협 없음): 소유권은 **유일키인 사번(employeeId) 기준으로 무조건 일치**시킨다.
#   담당자 명부(users 테이블)가 사번↔이름의 1:1 매핑을 그 자체로 보유하므로(한 행에 함께
#   있음 — 로그인 계정과 담당자 명부가 분리된 두 저장소이던 시절과 달리 별도 조회가 필요
#   없다), 로그인 사용자 자신의 행에서 바로 사번·이름 양쪽 식별자를 뽑아 매칭한다.
def _resolve_owner_identities(user: User, db: Session) -> set[str]:
    """로그인 사용자를 대표하는 모든 식별자 집합 — username(사번)/employee_id/표시 이름."""
    ids: set[str] = set()
    if user.username and user.username.strip():
        ids.add(user.username.strip())
    if user.employee_id and user.employee_id.strip():
        ids.add(user.employee_id.strip())
