...
바탕 테마 7종(`light` 슬레이트 **기본** / `dark` / `comfort` 화이트+딥그린 / `high-contrast` 고대비 /
`umber`·`umber-light` 유화 팔레트(움버+오커) / `plaster` 크림+슬레이트 블루 + `system`)과 강조색 6종(`blue`·`teal`·`green`·`amber`·`coral`·`violet`, `<html data-accent>` — 라이트·다크에만 적용) + 토큰 기반 시스템이다.
